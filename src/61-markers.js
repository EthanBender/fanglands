...
// ============================================================================
{
  ...
  // who lives inside which instance, so their marker can be hung on that instance's door
  const INSIDE = { brunhild: 'deepholm', thrain: 'deepholm', dagny: 'deepholm', orik: 'deepholm', hilde: 'deepholm' };
  const INSIDE_TILES = { LADDER_UP: 'deepholm', COAL_CART: 'deepholm' }; // moved underground with Deepholm
  const instanceDoor = id => {
    if (!window.INSTANCES || !INSTANCES.get) return null;
    const inst = INSTANCES.get(id); if (!inst) return null;
    return inst.door || (inst.step ? [inst.step[0], inst.step[1] - 1] : null);
  };
  ...
  function buildAll() {
    ...
    if (typeof tapPeople === 'function') {
      for (const p of tapPeople()) {
        ...
        if (label) { put(def.kind, Math.floor(p.x / TILE), Math.floor(p.y / TILE), label); placed.add(p.id); }
      }
    }
    // Some of those people now live inside an INSTANCE — Deepholm became its own map — so they are not on the
    // overworld to be found. Their marker goes on the door you go through to reach them, which is what a player
    // needs anyway: the mark says the smith is down the shaft, not that she has vanished.
    for (const id in INSIDE) {
      if (placed.has(id)) continue;
      const def = PEOPLE[id]; if (!def) continue;
      const label = def.shop ? shopName(def.shop) : def.label; if (!label) continue;
      // beside the door, never ON it: the door already carries the 'way down' mark and one marker per tile wins
      const door = instanceDoor(INSIDE[id]); if (!door) continue;
      const inst = INSTANCES.get(INSIDE[id]);
      const spot = (inst && inst.step) ? inst.step : [door[0], door[1] + 1];
      put(def.kind, spot[0], spot[1], label);
    }
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    try {
      ...
      { const doors = [], bad = [];
        ...
        const tileMark = (name, kind) => { const t = tileId(name); if (t === null) return 'no tile';
          // a tile that now lives inside an instance is not on the overworld map: say so plainly rather than failing
          if (window.INSTANCES && INSTANCES.get && INSIDE_TILES[name]) { const inst = INSTANCES.get(INSIDE_TILES[name]); if (inst && (inst.tiles || []).includes(t)) return 'inside ' + INSIDE_TILES[name]; } for (let y = 0; y < MAP_H; y++) for (let x = 0; x < MAP_W; x++) if (map[idx(x, y)] === t) { const m = at(x, y); return m && m.kind === kind ? true : 'missing at ' + x + ',' + y; } return 'no such tile on the map'; };
        ...
        // The ladder and the coal cart went underground when Deepholm became its own map (58-underground), so
        // they are no longer overworld tiles to mark — the SHAFT is what leads to them, and that must be marked.
        // The intent is unchanged: every way down off the overworld is marked, and nothing that moved is lost.
        const movedUnderground = ladder === 'inside deepholm' && cart === 'inside deepholm';
