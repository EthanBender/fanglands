...
// ============================================================================
{
  ...
  const _coffinFee = coffinFee;
  coffinFee = function () {
    const full = _coffinFee.apply(this, arguments);
    if (!full) return full; // kid mode and an empty chest both return 0 already
    const scaled = Math.ceil(full * (feeShare() / 0.25));
    return Math.min(scaled, feeCap());
  };
  // The per-item fee has to use the same share, or reclaiming one item at a time would cost more than the total.
  const _itemFee = itemFee;
  itemFee = function (id, qty) {
    const full = _itemFee.apply(this, arguments);
    return full ? Math.min(Math.ceil(full * (feeShare() / 0.25)), feeCap()) : full;
  };
