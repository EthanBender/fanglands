...
// ============================================================================
{
  ...
  // The chest's headline price and the per-item prices must agree EXACTLY, and the total must never pass the
  ...
  itemFee = function (id, qty) {
    ...
    // an item that is not in the chest (a caller asking the price of something else) keeps the plain scaled rate
    const v = ITEMS[id].value * qty;
    return v >= 20 ? Math.min(Math.ceil(v * feeShare()), feeCap()) : 0;
  };
  coffinFee = function () {
