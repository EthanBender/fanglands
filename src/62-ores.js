...
// ============================================================================
{
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    // How long a check waits for one landed swing. At the gate level a bronze pickaxe swings every 1.8 s
    // (108 steps) and lands 45 times in 100, so the wait must hold 20 or more swings: 900 steps held 8,
    // and 8 misses in a row (0.55^8, about 1 run in 120) failed the check on a healthy game.
    const MINE_WAIT = 2400;
    const swings = [];
    for (const t of TIERS) {
      ...
      if (started) swings.push({ key: t.key, need: player.action.need, tier: player.action.tier });
      const steps = F.untilAction(MINE_WAIT, () => player.skills.mining.xp > x0); player.action = null;
    ...
    // The wait itself, measured: the swing time and the pickaxe tier come off the action the game started,
    // the landing chance is the core's own rule written out a second time (35 in 100 at the gate, plus 10 a
    // pickaxe tier). Every at-level wait in this file uses MINE_WAIT, so this covers all of them.
    { const rows = swings.map(r => {
        const stepsPerSwing = Math.ceil(r.need * 60) + 1, n = Math.floor(MINE_WAIT / stepsPerSwing);
        const land = Math.min(0.9, 0.35 + r.tier * 0.1);
        return { key: r.key, stepsPerSwing, swings: n, land, missAll: Math.pow(1 - land, n) };
      });
      check(P + 'the mining checks wait long enough that bad luck cannot fail them: 20 or more swings at the gate level, and under 1 in 10000 to miss every one',
        rows.length === TIERS.length && rows.every(r => r.swings >= 20 && r.missAll < 1e-4),
        { wait: MINE_WAIT, rows }); }
    ...
    { const st = os(); const seen0 = st.seen.slice(); st.seen = [];
      ...
      if (spot) {
        ...
        F.tp(sx, sy); F.face(rx, ry); F.press('KeyE'); F.untilAction(MINE_WAIT, () => player.skills.mining.xp > a0); player.action = null;
        ...
        F.tp(sx, sy); F.face(rx, ry); F.press('KeyE'); F.untilAction(MINE_WAIT, () => player.skills.mining.xp > a1); player.action = null;
