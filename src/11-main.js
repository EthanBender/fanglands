...
window.FANGLANDS = {
  ...
  bfs(sx, sy, tx, ty) {
    ...
    while (qi < q.length) { const c = q[qi++]; const cx = c % W, cy = (c / W) | 0; if (cx === tx && cy === ty) break;
      for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) { const nx = cx + dx, ny = cy + dy; if (!inMap(nx, ny)) continue; const n = ny * W + nx; if (prev[n] !== -1) continue; if ((SOLID.has(map[n]) || pathBlocked(nx, ny, playerWho())) && !(nx === tx && ny === ty)) continue; prev[n] = c; q.push(n); } }
  ...
  walkTo(tx, ty, max = 4000) {
    ...
    // same heal rule as fight(): a road goblin must not kill the bot mid-walk (a death cascades through every later check)
    while (i < path.length && s < max) { if (player.hp < 7 && !window.__peace) { this.heals = (this.heals || 0) + 1; player.hp = player.maxHp; } const [wx, wy] = path[i]; const dx = tc(wx) - player.x, dy = tc(wy) - player.y; const last = i === path.length - 1; if (Math.hypot(dx, dy) < (last ? (SOLID.has(map[idx(wx, wy)]) ? 44 : 14) : 8)) { i++; continue; } this.step(this.held(dx, dy)); if (player.dead) return 'died'; s++; }
  ...
  fight(max, types = ['goblin']) {
    ...
    while (s < max) {
      ...
      if (d < 56) { pressed.add('Space'); this.step([]); for (let k = 0; k < 8; k++) this.step([]); s += 9; }
        ...
        if (stuck > 30) { stuck = 0; const r = this.walkTo(Math.floor(g.x / TILE), Math.floor(g.y / TILE), 400); s += typeof r === 'number' ? r : 400; if (r === 'nopath') { g.x = player.x + 60; g.y = player.y; } }
  ...
  selfTest() {
    ...
    // a build check owns its ground: no drop the earlier fights left lying (a coin on the tile answers Q with 'Pick that up first.'), no monster or villager close by
    const clutterFree = (x, y) => { const px = tc(x), py = tc(y); return !drops.some(d => dist(d.x, d.y, px, py) < 3 * TILE) && !monsters.some(m => !m.dead && dist(m.x, m.y, px, py) < 4 * TILE) && !NPCS.some(n => dist(n.px, n.py, px, py) < 3 * TILE); };
    // a check that spends a stock sets that stock up itself, so one unlucky gather cannot fail the checks after it
    const topUp = (id, n) => { const have = countItem(id); if (have < n) addItem(id, n - have); };
    ...
    // pin Math.random to a seeded stream until the returned function is called (a check whose outcome is combat or wander dice)
    const seedRandom = seed => { const r0 = Math.random; Math.random = mulberry32(seed); return () => { Math.random = r0; }; };
    // ---------- Chapter 1 ----------
    // Chapter 1's opening (spider, first blood, the signpost) is fought with dice: seeded from the new game to the signpost, so it replays the same every run
    ...
    HOOKS.kill.splice(HOOKS.kill.indexOf(thirdGoblin), 1);
    { const xp = firstXp !== null ? firstXp : player.skills.melee.xp; check('levelling pace: melee ≤ 5 after the first goblins', levelForXp(xp) <= 5, { melee: levelForXp(xp), xp, afterBot: player.skills.melee.xp }); }
    ...
    { const tree = F.nearestTile([T.TREE]); F.goAdjacent(tree.x, tree.y); F.fight(3000); F.goAdjacent(tree.x, tree.y);
      ...
      player.inv[axe] = saved; F.face(tree.x, tree.y); const near0 = monsters.filter(m => !m.dead && dist(m.x, m.y, player.x, player.y) < 160).map(m => m.type + ':' + m.state); notice = null; F.press('KeyE'); const a0 = player.action && player.action.type, n0 = notice && notice.text;
    ...
    F.fight(3000); { const o = openSpot(Math.floor(player.x / TILE), Math.floor(player.y / TILE), clutterFree); F.tp(o.x, o.y); } topUp('plank', 4); player.facing = { x: 1, y: 0 }; { const ft = frontTile(player, 40); F.press('KeyQ'); F.sim(3, []); check('place a plank', countItem('plank') === 3 && tileAt(ft.tx, ft.ty) === T.PLANK, { after: tileAt(ft.tx, ft.ty) }); if (tileAt(ft.tx, ft.ty) !== T.PLANK) { changeTile(ft.tx, ft.ty, T.PLANK); if (countItem('plank') > 3) removeItem('plank', countItem('plank') - 3); } F.press('KeyE'); F.sim(2, []); check('pick a placed plank back up', countItem('plank') === 4 && tileAt(ft.tx, ft.ty) === T.GRASS, { tile: tileAt(ft.tx, ft.ty) }); }
    ...
    { const bag = player.inv, w0 = player.equip.weapon; player.inv = new Array(INV_SLOTS).fill(null); player.inv[0] = { id: 'iron_dagger', qty: 1 }; player.equip.weapon = 'wooden_sword';
      openPanel('inventory'); render(); F.clickButton('slot0'); F.clickButton('Equip'); const eq = player.equip.weapon === 'iron_dagger' && countItem('wooden_sword') === 1 && countItem('iron_dagger') === 0; F.clickButton('eqweapon'); closePanel();
      check('equipment slots: equip swaps the old weapon back to the pack, unequip works', eq && player.equip.weapon === null && countItem('iron_dagger') === 1 && countItem('wooden_sword') === 1, { eq, weapon: player.equip.weapon, dagger: countItem('iron_dagger'), sword: countItem('wooden_sword') });
    ...
    { const o = openSpot(40, 20); F.tp(o.x, o.y); const gob = monsters.find(m => m.type === 'goblin' && !m.dead); gob.x = player.x + 120; gob.y = player.y; gob.home = { x: gob.x, y: gob.y }; gob.state = 'idle'; gob.hp = gob.maxHp; gob.stunT = 0; gob.wanderT = 99; gob.wander = { x: 0, y: 0 }; player.facing = { x: 1, y: 0 };
      ...
      // a loot drop from the fight may sit on the trap's tile ("Pick that up first."), so the drops there are cleared before placing; the check brings its own trap.
      // Keep this comment on its own line: trailing it on the line below once swallowed the placement check and left a live trap on the map.
      // a trap does 12 and a goblin has 12 hp, so a full-health goblin dies on it and "stuns it" would be read off a corpse: lend it 20 hp for this check so it lives to show the stun, then give the hp back
      const trapMh0 = gob.maxHp; gob.maxHp = trapMh0 + 20;
      gob.dead = false; gob.hp = gob.maxHp; gob.state = 'idle'; gob.stunT = 0; gob.x = player.x + 200; gob.y = player.y; player.facing = { x: 1, y: 0 }; const ft = frontTile(player, 40); drops = drops.filter(d => !circleHitsTile(d.x, d.y, 8, ft.tx, ft.ty)); if (!countItem('goblin_trap')) give('goblin_trap', 1); placeAction('goblin_trap');
      const placed = tileAt(ft.tx, ft.ty) === T.TRAP; gob.x = tc(ft.tx); gob.y = tc(ft.ty); F.sim(3, []); check('goblin trap: placed, springs on a goblin, stuns it', placed && gob.hp === gob.maxHp - 12 && !gob.dead && gob.stunT > 0 && tileAt(ft.tx, ft.ty) !== T.TRAP, { placed, hp: gob.hp, alive: !gob.dead, stun: gob.stunT });
