...
window.FANGLANDS = {
  ...
  selfTest() {
    ...
    { const tree = F.nearestTile([T.TREE]); F.goAdjacent(tree.x, tree.y); F.fight(3000); F.goAdjacent(tree.x, tree.y); F.press('KeyE'); F.untilAction(900, () => countItem('wood') >= 2); } peace(false); // the second log is timed too: a goblin's hit cancels the swing and every plank check after it fails
    ...
    { player.inv = new Array(INV_SLOTS).fill(null); give('coins', 100); give('goblin_scrap', 2); give('wood', 3); player.bedSpawn = null;
      ...
      const share = window.ECONOMY ? ECONOMY.feeShare() : 0.25, want = Math.min(Math.ceil(8 * (share / 0.25)), window.ECONOMY ? ECONOMY.feeCap() : 8);
