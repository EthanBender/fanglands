...
// ============================================================================
{
  ...
  // after a death: wait to wake, then go to Death's house in Thistledown and buy the pack back (it holds whatever the
  // knight was carrying: dung, scales, silk). The note says whether the chest really came back, not just that he went.
  function afterDeath() {
    PLAY.deaths++; note(`died at ${ptx()},${pty()}`); let n = 0; while (player.dead && n++ < 400) tick();
    if (!deathKeep) return;
    const purse = coins() + (deathKeep.items.find(s => s.id === 'coins') || { qty: 0 }).qty;
    if (purse < coffinFee()) { note(`left the pack with Death: fee ${coffinFee()}, coins ${purse}`); return; }
    const death = NPCS.find(o => o.id === 'death2'); if (!death) return;
    for (let k = 0; k < 3 && deathKeep && !player.dead; k++) {
      closePanel(); drain();
      if (!walk(death.x, death.y + 1)) break;
      F().face(death.x, death.y); F().press('KeyE'); PLAY.steps++;
      if (panel === 'coffin') F().clickButton('Reclaim everything');
      closePanel(); drain();
    }
    note(deathKeep ? `could not reclaim the pack from Death (${deathKeep.items.map(s => s.id + ' ' + s.qty).join(', ')})` : 'reclaimed the pack from Death');
  }
  ...
  // `tries` counts fights only. When none of the type is alive the knight goes back to `base` and waits there, eating
  // when he needs to, until one comes back: base must be a safe tile more than 4 tiles from every home, because a knight
  // standing that close to a home holds that monster's respawn back (src/07-update.js). The default base is the tile
  // the hunt starts from; null means wait where he stands. A death while waiting is handled like any other death
  // (afterDeath buys the pack back from Death). The waiting is capped by time: twice the monster's respawn per
  // item still missing when the hunt starts.
  function hunt(type, itemId, want, tries, waitTicks = 600, base = [ptx(), pty()]) {
    let n = 0, waited = 0; const kills0 = player.kills;
    const maxWait = 2 * (MONSTER_DEFS[type].respawn || 60) * 60 * Math.max(1, want - countItem(itemId));
    const alive = () => monsters.some(o => o.type === type && !o.dead);
    while (countItem(itemId) < want && n < tries && !over()) {
      heal(); if (player.dead) { afterDeath(); continue; }
      const m = monsters.filter(o => o.type === type && !o.dead).sort((a, b) => dist(a.x, a.y, player.x, player.y) - dist(b.x, b.y, player.x, player.y))[0];
      if (!m) {
        if (waited >= maxWait) { note(`hunt ${type}: none came back in ${waited} ticks of waiting`); break; }
        if (base && (ptx() !== base[0] || pty() !== base[1])) walk(base[0], base[1]);
        for (let k = 0, stop = Math.min(waitTicks, maxWait - waited); k < stop && !alive() && !over(); k++) {
          heal(); tick(); waited++;
          if (player.dead || chasing(4 * TILE).length) break;
        }
        if (player.dead) afterDeath();
        else if (chasing(4 * TILE).some(o => o.type !== type)) { const r = brawl(HOSTILE, 3000); waited += typeof r === 'number' ? r : 3000; }
        continue;
      }
      n++;
      const w = walk(Math.floor(m.x / TILE), Math.floor(m.y / TILE), true, 3000); const r = brawl([type], 20000, 8 * TILE);
      if (n <= 2 || n % 5 === 0) note(`hunt ${type} #${n}: walk ${w}, brawl ${r}, ${itemId} ${countItem(itemId)}/${want}, kills +${player.kills - kills0}, free slots ${player.inv.filter(s => !s).length}, waited ${waited}`);
    }
    return countItem(itemId) >= want;
  }
  ...
  function play(budget = 1500000) {
    ...
    // the hunt waits for respawns in Dunstan's yard, where the knight stands after talking to him: away from the lava,
    // far from the green dragon, more than 4 tiles from every drake's home. The scale hunt later waits there too.
    let drakeBase = null;
    untilStage(14, () => {
      drakeBase = [ptx(), pty()]; hunt('ash_drake', 'dragon_dung', 5, 14, 600, drakeBase);
    ...
    untilStage(15, () => {
      // Wren's silk: five strands from the cave spiders. They are harmless and live in a dead-end cave, so that hunt
      // waits where it stands (base null) rather than walking back to Wren
      if (quest.wren !== 'done') { talk('wren'); drain(); hunt('spider', 'spider_silk', 5, 60, 300, null); talk('wren'); drain(); }
      ...
      // dragon scales from the drakes (one in four kills or so), waiting for respawns in Dunstan's yard
      hunt('ash_drake', 'dragon_scale', 5, 60, 600, drakeBase || undefined);
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    { const drakes = monsters.filter(m => m.type === 'ash_drake'), dun = NPCS.find(o => o.id === 'dunstan'), death = NPCS.find(o => o.id === 'death2');
      if (drakes.length && dun && death) {
        const saveBot = () => ({ x: player.x, y: player.y, hp: player.hp, equip: { ...player.equip }, maxHp: player.maxHp, dead: player.dead, inv: player.inv.map(s => s ? { ...s } : null), skills: JSON.stringify(player.skills), kills: player.kills, deaths: player.deaths,
          dk: deathKeep, quest: JSON.stringify(quest), drops: drops.slice(), peace: window.__peace, respawn: MONSTER_DEFS.ash_drake.respawn,
          play: { steps: PLAY.steps, budget: PLAY.budget, deaths: PLAY.deaths, meals: PLAY.meals, healed: PLAY.healed, log: PLAY.log.slice(), last: PLAY.last },
          drakes: drakes.map(m => ({ m, dead: m.dead, hp: m.hp, maxHp: m.maxHp, respawnT: m.respawnT, deadT: m.deadT, state: m.state, x: m.x, y: m.y, angry: m.angry })) });
        const restoreBot = k => { closePanel(); drain(); Object.assign(player, { x: k.x, y: k.y, hp: k.hp, equip: k.equip, maxHp: k.maxHp, dead: k.dead, inv: k.inv, skills: JSON.parse(k.skills), kills: k.kills, deaths: k.deaths, action: null });
          deathKeep = k.dk; for (const key of Object.keys(quest)) delete quest[key]; Object.assign(quest, JSON.parse(k.quest)); drops = k.drops; window.__peace = k.peace; MONSTER_DEFS.ash_drake.respawn = k.respawn;
          Object.assign(PLAY, k.play); for (const d of k.drakes) { const { m, ...rest } = d; Object.assign(m, rest); } };
        // all four drakes dead; only the first can come back soon, and the others not during the check
        const downAll = soon => { for (const m of drakes) { m.dead = true; m.hp = 0; m.deadT = 0; m.respawnT = 99999; m.maxHp = 1; m.state = 'idle'; } drakes[0].respawnT = soon; };
        const base = [dun.x, dun.y - 1];
        // 1. the knight stands on the only drake's home, where standing holds its respawn back, with one try: before,
        //    the bot waited on the spot and the wait spent the try; now it walks back to the base and only fights count
        { const k = saveBot(); let ok = false, err = null, dung = 0, at = null;
          // the knight is given the level and blade the bot has by stage 13 (melee 22, an iron dagger) so the fight is short
          try { h.peace(true); drain(); player.inv = new Array(INV_SLOTS).fill(null); player.dead = false; player.skills.melee.xp = Math.max(player.skills.melee.xp, XP_TABLE[22]); player.equip.weapon = 'iron_dagger'; player.hp = player.maxHp; downAll(2);
            F.tp(Math.floor(drakes[0].home.x / TILE), Math.floor(drakes[0].home.y / TILE)); PLAY.budget = PLAY.steps + 20000;
            ok = hunt('ash_drake', 'dragon_dung', 1, 1, 600, base); dung = countItem('dragon_dung'); at = [ptx(), pty()]; }
          catch (e) { err = String(e && e.message); }
          const log = PLAY.log.slice(k.play.log.length); restoreBot(k);
          check(P + "the bot's hunt waits for a respawn at a safe base, and an empty wait does not use up a try (the stage 13 dung)", ok && dung === 1 && !err, { ok, dung, at, err, log }); }
        // 2. the knight carries dung and dies while he waits: before, the wait ran on and the pack stayed with Death;
        //    now afterDeath runs and he buys the pack back. The base here is Death's own house so the check stays short,
        //    and the drakes' respawn is cut to one second so the capped wait ends soon after.
        { const k = saveBot(), killHook = () => { ticks++; if (!fired && ticks > 30) { fired = true; player.hp = 0; die(); } }; let ticks = 0, fired = false, ok = null, err = null, dung = 0;
          try { h.peace(true); drain(); player.inv = new Array(INV_SLOTS).fill(null); player.dead = false; player.hp = player.maxHp; addItem('dragon_dung', 1); addItem('coins', 200); deathKeep = null;
            downAll(99999); MONSTER_DEFS.ash_drake.respawn = 1; F.tp(death.x, death.y + 1); PLAY.budget = PLAY.steps + 20000;
            HOOKS.update.push(killHook); ok = hunt('ash_drake', 'dragon_dung', 2, 1, 600, [death.x, death.y + 1]); dung = countItem('dragon_dung'); }
          catch (e) { err = String(e && e.message); }
          const i = HOOKS.update.indexOf(killHook); if (i >= 0) HOOKS.update.splice(i, 1);
          const deaths = PLAY.deaths - k.play.deaths, kept = deathKeep, alive = !player.dead, log = PLAY.log.slice(k.play.log.length); restoreBot(k);
          check(P + "the bot's hunt: a death while waiting is followed by the walk to Death, and the dung he carried comes back", fired && deaths === 1 && !kept && dung === 1 && alive && ok === false && !err, { fired, deaths, kept: kept && kept.items, dung, alive, ok, err, log }); }
      } }
