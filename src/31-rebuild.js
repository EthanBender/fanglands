...
// ============================================================================
{
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    // the wander is dice (update hook above: a 2-6 s hold per roll, 3 in 10 holds idle, a walk home from 4 tiles out), so one
    // sampled frame proves nothing: a villager who wandered can be back on its home tile at frame 600 (~1 run in 100).
    // Watch every frame instead: EACH villager must get more than 12 px from home, and NEITHER may stand outside the square
    // on any frame. At least 600 frames as before, then on until both have wandered, capped at 4800 (80 s). An idle villager
    // re-rolls within 6 s, so 80 s is at least 13 rolls: a false failure is at most 2 x 0.3^13 = 3.2e-7.
    { F.tp(150, 60); const went = villagers.map(() => false); let stayed = true, f = 0;
      for (; f < 4800 && (f < 600 || !went.every(Boolean)); f++) { F.step([]); villagers.forEach((w, k) => { if (dist(w.px, w.py, w.home.x, w.home.y) > 12) went[k] = true; if (!sqIn(Math.floor(w.px / TILE), Math.floor(w.py / TILE))) stayed = false; }); }
      render(); const moved = villagers.length > 0 && went.every(Boolean);
      ...
      check('rebuild: villagers wander inside the square; E on the square Nell gets a rebuilding line', moved && stayed && spoke, { moved, went, frames: f, stayed, spoke, who: dialog.cur && dialog.cur.who }); drain(); }
