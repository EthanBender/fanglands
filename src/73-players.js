...
// ============================================================================
{
  ...
  HOOKS.panel.friends = (g, narrow) => {
    ...
    for (const o of page) {
      ...
      g.textAlign = 'left'; g.fillStyle = admin ? GOLD : '#e6edf3'; g.fillText(shownName, px + 30, y + 20);
      const nw = g.measureText(shownName).width;
      if (admin) adminPill(g, px + 30 + nw + 6, y + 16, 9);
      const lvText = 'lv ' + (o.lv || 1), lvX = px + 30 + nw + 8 + pillW; g.font = '11px sans-serif';
      if (lvX + g.measureText(lvText).width <= px + 30 + roomW) { g.fillStyle = '#9aa3b2'; g.textAlign = 'left'; g.fillText(lvText, lvX, y + 20); }
