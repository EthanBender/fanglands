...
// ============================================================================
{
  ...
  function buildGeom(g) {
    ...
    const w = Math.min(VW - 20, 600), inner = w - 36, wide = inner >= 480, f = K.SENT(13), lh = K.lineH(f);
    ...
    const pagerH = R + 8, room0 = VH - 20 - 62 - 12 - top - bottom;
  ...
  // the panel audit's scene (PANEL_KIT, run from 63-house): on the island (as the panel sees it) with planks and stone
  ...
  HOOKS.selfTest.push((check, F, h) => PANEL_KIT.audit(check, F, h));
}
