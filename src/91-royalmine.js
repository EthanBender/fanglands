...
// ============================================================================
{
  ...
  let bonusWas, bonusOn = false;
  HOOKS.kill.unshift(m => { if (m && NO_BONUS.has(m.type)) { bonusWas = window.__companionHit; window.__companionHit = true; bonusOn = true; } });
  HOOKS.kill.push(m => {
    if (bonusOn) { window.__companionHit = bonusWas; bonusOn = false; }
  ...
  function inlay(g, tx, ty) {
    const cx = tx * TILE + TILE / 2, cy = ty * TILE + TILE / 2, v = DECOR[ty * W + tx] || 0;
    g.fillStyle = 'rgba(40,28,10,0.35)'; g.beginPath(); g.moveTo(cx, cy - 8); g.lineTo(cx + 8, cy); g.lineTo(cx, cy + 8); g.lineTo(cx - 8, cy); g.closePath(); g.fill();
    g.fillStyle = GOLD;
    if (v === 0) { g.beginPath(); g.moveTo(cx, cy - 5.5); g.lineTo(cx + 5.5, cy); g.lineTo(cx, cy + 5.5); g.lineTo(cx - 5.5, cy); g.closePath(); g.fill(); }
    else if (v === 1) { g.strokeStyle = GOLD; g.lineWidth = 2; g.beginPath(); g.arc(cx, cy, 4.5, 0, 7); g.stroke(); g.beginPath(); g.arc(cx, cy, 1.6, 0, 7); g.fill(); }
    else if (v === 2) { g.beginPath(); for (let k = 0; k < 8; k++) { const a = k / 8 * Math.PI * 2, r = k % 2 ? 2.2 : 6; g.lineTo(cx + Math.cos(a) * r, cy + Math.sin(a) * r); } g.closePath(); g.fill(); }
    else { g.fillRect(cx - 4, cy - 4, 8, 8); g.fillStyle = GOLD_D; g.fillRect(cx - 1.5, cy - 1.5, 3, 3); }
    g.fillStyle = GOLD_L; g.beginPath(); g.moveTo(cx, cy - 5); g.lineTo(cx + 1.8, cy - 1.8); g.lineTo(cx, cy - 0.5); g.lineTo(cx - 1.8, cy - 1.8); g.closePath(); g.fill();
  }
  function chamberCrack(g, tx, ty, glow) {
    if (hh(tx, ty, 3) > 0.34) return;
    const x = tx * TILE, y = ty * TILE, a0 = hh(tx, ty, 5) * Math.PI * 2;
    const pts = []; let px = x + 8 + hh(tx, ty, 7) * 32, py = y + 8 + hh(ty, tx, 9) * 32;
    for (let k = 0; k < 4; k++) { pts.push([px, py]); const a = a0 + (hh(tx + k, ty, 11) - 0.5) * 1.6; px = clamp(px + Math.cos(a) * 11, x + 2, x + TILE - 2); py = clamp(py + Math.sin(a) * 11, y + 2, y + TILE - 2); }
    g.lineCap = 'round'; g.lineJoin = 'round';
    g.strokeStyle = '#3a120c'; g.lineWidth = 3.4; g.beginPath(); pts.forEach(([a, b], i) => i ? g.lineTo(a, b) : g.moveTo(a, b)); g.stroke();
    g.strokeStyle = `rgba(255,${90 + Math.round(60 * glow)},50,${(0.35 + 0.5 * glow).toFixed(3)})`; g.lineWidth = 1.5; g.stroke();
  }
  ...
  function drawVein(g, tx, ty) {
    ...
    const left = lit ? NUM.VEIN_TAKE - (run.quota[veinKey(ms, tx, ty)] || 0) : 0;
    const [fx, fy] = veinFace(tx, ty), cx = x + TILE / 2 + fx * 10, cy = y + TILE / 2 + fy * 10;
    const pulse = 0.65 + 0.35 * Math.sin(time * 6 + tx);
    if (lit) {
      const gr = g.createRadialGradient(cx, cy, 4, cx, cy, 62); gr.addColorStop(0, `rgba(255,110,70,${(0.45 * pulse + 0.4 * flash).toFixed(3)})`); gr.addColorStop(1, 'rgba(255,90,60,0)');
      g.fillStyle = gr; g.beginPath(); g.arc(cx, cy, 62, 0, 7); g.fill();
    }
    const core = lit ? `rgb(255,${Math.round(120 + 90 * pulse + 40 * flash)},${Math.round(80 + 60 * flash)})` : '#6a1e14', edge = lit ? '#ff5a3c' : '#3e120c';
    g.lineCap = 'round'; g.lineJoin = 'round';
    for (let s = 0; s < 3; s++) {
    ...
    // the crystals you can still break off this glow (four when it lights, fewer as you take them)
    g.fillStyle = core;
    for (let j = 0; j < (lit ? left : 2); j++) { const a = j * 1.6 + hh(ty, tx, j) * 1.2, r = 7 + j * 2; g.beginPath(); g.moveTo(cx + Math.cos(a) * r, cy + Math.sin(a) * r - 5); g.lineTo(cx + Math.cos(a) * r + 3.5, cy + Math.sin(a) * r + 2); g.lineTo(cx + Math.cos(a) * r - 3.5, cy + Math.sin(a) * r + 2); g.closePath(); g.fill(); }
    if (lit) for (let k = 0; k < 3; k++) { const ph = (time * 1.4 + k / 3 + tx * 0.1) % 1; g.fillStyle = `rgba(255,${200 - k * 30},120,${(1 - ph).toFixed(3)})`; g.beginPath(); g.arc(cx + Math.sin(time * 3 + k * 2) * 8, cy - ph * 26, 1.5, 0, 7); g.fill(); }
  }
  ...
  function drawGiantOre(g, e, hurt) {
    ...
    const S = gd.size * TILE, mith = gd.metal === 'mithril', n = mith ? 9 : 11, id = mith ? 7 : 13;
    ...
    const shake = st === 'waking' ? 4 : st === 'stir' ? 2 : 0;
    g.save(); g.translate(shake ? Math.sin(time * 47 + e.x) * shake : 0, (shake ? Math.cos(time * 39) * shake * 0.5 : 0) - S * 0.06);
    const pts = []; for (let k = 0; k < n; k++) { const a = -Math.PI / 2 + k / n * Math.PI * 2, rr = 0.86 + hh(id, k, 3) * 0.14; pts.push([Math.cos(a) * S * 0.46 * rr, Math.sin(a) * S * 0.42 * rr]); }
    const body = hurt ? ['#e8d0d0', '#f4dede', '#c8b0b0'] : mith ? ['#5f7aa8', '#7593c6', '#435c86'] : ['#5a4a8e', '#6f5fb0', '#3f3468'];
    const vein = mith ? '#bcd4f5' : '#c8b0ff', deep = mith ? '#3d5680' : '#3a2e5e', glowC = mith ? '160,210,255' : '210,180,255';
    const outline = () => { g.beginPath(); pts.forEach(([x, y], i) => i ? g.lineTo(x, y) : g.moveTo(x, y)); g.closePath(); };
    const split = frac < 0.33 ? 5 : 0;
    for (const half of split ? [-1, 1] : [0]) {
      g.save();
      if (half) { g.beginPath(); g.rect(half < 0 ? -S : 0, -S, S, S * 2); g.clip(); g.translate(half * split, 0); }
      const bg = g.createLinearGradient(-S / 2, -S / 2, S / 2, S / 2); bg.addColorStop(0, body[1]); bg.addColorStop(1, body[2]);
      g.fillStyle = bg; outline(); g.fill(); g.strokeStyle = 'rgba(0,0,0,0.4)'; g.lineWidth = 2; g.stroke();
      g.fillStyle = 'rgba(255,255,255,0.13)'; g.beginPath(); g.moveTo(pts[n - 1][0] * 0.8, pts[n - 1][1] * 0.8); g.lineTo(pts[0][0] * 0.85, pts[0][1] * 0.85); g.lineTo(pts[1][0] * 0.6, pts[1][1] * 0.6); g.lineTo(-S * 0.05, -S * 0.05); g.closePath(); g.fill();
      g.lineCap = 'round'; g.lineJoin = 'round';
      // thick veins of its metal
      for (let v = 0; v < (mith ? 3 : 2); v++) {
        const a = hh(id, v, 11) * 6.28;
        g.strokeStyle = deep; g.lineWidth = 6; g.beginPath(); g.moveTo(Math.cos(a) * S * 0.34, Math.sin(a) * S * 0.28);
        g.quadraticCurveTo(Math.cos(a + 1.8) * S * 0.12, Math.sin(a + 1.8) * S * 0.1, Math.cos(a + 3) * S * 0.32, Math.sin(a + 3) * S * 0.26); g.stroke();
        g.strokeStyle = vein; g.lineWidth = 3; g.stroke(); g.strokeStyle = 'rgba(245,248,255,0.8)'; g.lineWidth = 1; g.stroke();
      }
      if (!mith) {
        // stormstone: a crown of crystals, and lightning in the rock
        for (let k = 0; k < 5; k++) { const bx = -S * 0.2 + k * S * 0.1, by = -S * 0.3 + Math.abs(k - 2) * 5, h = S * (0.18 - Math.abs(k - 2) * 0.03); g.fillStyle = k % 2 ? '#c8b0ff' : '#a98ae8'; g.beginPath(); g.moveTo(bx - 6, by); g.lineTo(bx - 2, by - h); g.lineTo(bx + 3, by - h * 0.92); g.lineTo(bx + 6, by); g.closePath(); g.fill(); }
        if (Math.sin(time * 5 + e.x) > 0.6) { g.strokeStyle = 'rgba(230,215,255,0.9)'; g.lineWidth = 1.6; g.beginPath(); g.moveTo(-S * 0.2, -S * 0.05); g.lineTo(-S * 0.06, S * 0.04); g.lineTo(-S * 0.12, S * 0.1); g.lineTo(S * 0.06, S * 0.2); g.stroke(); }
      }
      if (frac > 0.66) {
        for (let k = 0; k < 5; k++) { const a = hh(id, k, 21) * 6.28, d = hh(k, id, 22) * S * 0.28; g.fillStyle = `rgba(240,246,255,${(0.35 + 0.55 * Math.max(0, Math.sin(time * 3 + k))).toFixed(3)})`; g.beginPath(); g.arc(Math.cos(a) * d, Math.sin(a) * d, 1.8, 0, 7); g.fill(); }
      } else {
        // deep cracks, and chips gone from the rim
        g.strokeStyle = 'rgba(14,10,18,0.85)'; g.lineWidth = 3; g.beginPath(); g.moveTo(-S * 0.3, -S * 0.1); g.lineTo(-S * 0.12, -S * 0.02); g.lineTo(-S * 0.05, S * 0.14); g.moveTo(S * 0.1, -S * 0.3); g.lineTo(S * 0.18, -S * 0.12); g.lineTo(S * 0.3, -S * 0.06); g.stroke();
        g.fillStyle = '#231c24'; for (const k of [2, 6]) { const [px, py] = pts[k % n]; g.beginPath(); g.moveTo(px, py); g.lineTo(px * 0.78 + 4, py * 0.78); g.lineTo(px * 0.86, py * 0.86 + 6); g.closePath(); g.fill(); }
      }
      g.restore();
    }
    if (split) {
      // split open: the seam glows through the gap, and grit falls out of it
      const gr = g.createLinearGradient(0, -S * 0.4, 0, S * 0.4); gr.addColorStop(0, `rgba(${glowC},0)`); gr.addColorStop(0.5, `rgba(${glowC},0.85)`); gr.addColorStop(1, `rgba(${glowC},0)`);
      g.fillStyle = gr; g.fillRect(-split, -S * 0.42, split * 2, S * 0.84);
      for (let k = 0; k < 4; k++) { const ph = (time * 1.3 + k / 4) % 1; g.fillStyle = `rgba(150,140,125,${(1 - ph).toFixed(3)})`; g.beginPath(); g.arc(Math.sin(k * 2.1) * 4, -S * 0.1 + ph * S * 0.5, 1.6, 0, 7); g.fill(); }
    }
    if (st === 'stir' || st === 'waking') {
      // something inside is waking: two eyes, glowing in its metal's colour, in the deepest crack
      const wide = st === 'waking' ? 1.6 : 1, blink = st === 'stir' && Math.sin(time * 2.3 + e.x) > 0.94 ? 0.2 : 1;
      const eg = g.createRadialGradient(0, -S * 0.04, 1, 0, -S * 0.04, S * 0.3); eg.addColorStop(0, `rgba(${glowC},0.5)`); eg.addColorStop(1, `rgba(${glowC},0)`); g.fillStyle = eg; g.beginPath(); g.arc(0, -S * 0.04, S * 0.3, 0, 7); g.fill();
      g.fillStyle = mith ? '#dff0ff' : '#f0e4ff'; for (const ex of [-8, 8]) { g.beginPath(); g.ellipse(ex * wide, -S * 0.06, 4 * wide, 3 * blink * wide, 0, 0, 7); g.fill(); }
      g.fillStyle = mith ? '#5aa0ff' : '#b070ff'; for (const ex of [-8, 8]) { g.beginPath(); g.arc(ex * wide, -S * 0.06, 1.6 * blink * wide, 0, 7); g.fill(); }
    }
    g.restore();
  }
  ...
  function drawStones(g) {
    for (const s of run.stones) {
      ...
      for (let j = 4; j >= 1; j--) { const [xx, yy] = at(Math.max(0, k - j * 0.05)); g.fillStyle = `rgba(255,${130 + j * 15},60,${(0.45 - j * 0.09).toFixed(3)})`; g.beginPath(); g.arc(xx, yy, 5 - j * 0.8, 0, 7); g.fill(); }
      const [x, y] = at(k);
      const gr = g.createRadialGradient(x, y, 1, x, y, 18); gr.addColorStop(0, 'rgba(255,210,120,0.9)'); gr.addColorStop(1, 'rgba(255,120,40,0)'); g.fillStyle = gr; g.beginPath(); g.arc(x, y, 18, 0, 7); g.fill();
      g.fillStyle = '#ff9a3a'; g.beginPath(); g.arc(x, y, 6, 0, 7); g.fill(); g.fillStyle = '#fff0c0'; g.beginPath(); g.arc(x - 1.5, y - 1.5, 2.2, 0, 7); g.fill();
    }
  }
  ...
  function drawMarker(g) {
    ...
    const b = Math.abs(Math.sin(time * 4)) * 8, x = tg.x, y = tg.y - b;
    g.fillStyle = 'rgba(0,0,0,0.5)'; g.beginPath(); g.moveTo(x - 13, y - 16); g.lineTo(x + 13, y - 16); g.lineTo(x, y + 2); g.closePath(); g.fill();
    g.fillStyle = '#f5c542'; g.beginPath(); g.moveTo(x - 11, y - 17); g.lineTo(x + 11, y - 17); g.lineTo(x, y); g.closePath(); g.fill();
    g.fillStyle = '#fff2b0'; g.beginPath(); g.moveTo(x - 5, y - 15); g.lineTo(x + 1, y - 15); g.lineTo(x - 2, y - 9); g.closePath(); g.fill();
  }
  ...
  HOOKS.draw.push((g, items) => {
    ...
    const socket = (px, ty) => { const y = ty * TILE; g.fillStyle = '#1a1614'; g.beginPath(); g.arc(px, y + 12, 4.2, 0, 7); g.fill(); g.strokeStyle = '#8a8d95'; g.lineWidth = 1.2; g.stroke(); };
    if (slide.on && DH.drawThrone) items.push({ y: 23 * TILE + TILE - 6, draw: () => { const fx = 14 + 2 * ease(slide.t / NUM.SLIDE); DH.drawThrone(g, fx, 23); if (R().stage >= 4) socket(tc(fx), 23); } });
    else if (R().stage >= 4 && tileAt(16, 23) === DW_THRONE) items.push({ y: 23 * TILE + TILE - 6 + 0.01, draw: () => socket(tc(16), 23) });
