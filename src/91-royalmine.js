...
// ============================================================================
{
  ...
  function keeperGolem(dt) {
    ...
    if (AWAKE.has(rm.phase)) {
      ...
      if (rm.lingT <= 0) {
        ...
        for (let k = 0; k < (half ? NUM.LING_HALF_COUNT : 1) && alive + k < NUM.LING_MAX; k++) spawnLittle(rm.heap++);
      }
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    const S0 = {
      ...
      num: clone(NUM), peace: window.__peace, th: th0 ? { x: th0.x, px: th0.px, py: th0.py } : null, drops: drops.slice(), dhSum: dhSum(), companion: window.__companionHit, hadCompanion: '__companionHit' in window,
    };
    ...
    try {
      ...
      player.x = S0.px; player.y = S0.py; drops = S0.drops; if (S0.hadCompanion) window.__companionHit = S0.companion; else delete window.__companionHit;
