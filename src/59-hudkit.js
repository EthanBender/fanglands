  ...
  function drawOverlays(g, L) {
    ...
    for (const c of FRAME.coach || []) {
      ...
      if (c.at && c.at.sx != null) { sx = c.at.sx; sy = c.at.sy; }
        ...
        sx = Math.round(c.at.x - cam.x); sy = Math.round(c.at.y - cam.y) - (c.at.lift || 30);
        // above the thing unless a lane or a HUD piece is there; then below it
        const w0 = 140, above = { x: sx - w0 / 2, y: sy - 38, w: w0, h: 38 };
        if (clash(above)) sy = Math.round(c.at.y - cam.y) + (c.at.lift || 30) + 38;
      } else continue;
