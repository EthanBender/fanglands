...
// ============================================================================
{
  ...
  set('steel_battleaxe', (g, size, item) => {                     // two bits, one each side of the haft
    ...
    g.fillStyle = item.color;
    g.beginPath(); g.moveTo(3.4, -2.6); g.lineTo(6.4, -6.6); g.quadraticCurveTo(9.4, 0, 6.4, 6.6); g.lineTo(3.4, 2.6);
    g.quadraticCurveTo(5.4, 0, 3.4, -2.6); g.closePath(); g.fill(); line(g, OUT, 1.2); g.stroke();
    g.beginPath(); g.moveTo(2.6, -2.6); g.lineTo(-0.4, -6.6); g.quadraticCurveTo(-3.6, 0, -0.4, 6.6); g.lineTo(2.6, 2.6);
    g.quadraticCurveTo(0.6, 0, 2.6, -2.6); g.closePath(); g.fill(); g.stroke();
  ...
  set('golden_bread', (g, size, item) => {                        // a plaited loaf, glazed, and it catches the light
    g.fillStyle = item.color;                                                                                       // the body of the loaf
    g.beginPath(); g.ellipse(0, 0.4, 9, 5.4, 0, 0, 7); g.fill(); line(g, OUT, 1.2); g.stroke();
    line(g, dk(0.34), 1.4);                                                                                          // the seams the plait folds into
    for (let k = 0; k < 5; k++) { const x = -6.4 + k * 3.2;
      g.beginPath(); g.moveTo(x - 2, 4.4); g.quadraticCurveTo(x, 0.4, x + 2, -4.4); g.stroke(); }
    g.fillStyle = lt(0.34);                                                                                          // the crown of each strand, catching the glaze
    for (let k = 0; k < 5; k++) { const x = -5.4 + k * 3.2; g.beginPath(); g.ellipse(x, -0.6, 1.2, 2.6, 0.5, 0, 7); g.fill(); }
  ...
  set('blackiron_ore', (g, size, item) => {                       // a cubic crystal set in a sooty rock
    cobble(g, item.color);
    g.fillStyle = '#c2c9d2';                                                                                       // the cube, drawn in three faces, bright enough to find
    ...
    g.fillStyle = '#eef2f6'; g.beginPath(); g.moveTo(-1.4, -3.4); g.lineTo(0.4, -6.4); g.lineTo(5.4, -7); g.lineTo(3.4, -4.4); g.closePath(); g.fill(); g.stroke();
    g.fillStyle = '#8f96a3'; g.beginPath(); g.moveTo(3.4, -4.4); g.lineTo(5.4, -7); g.lineTo(5.4, -2.4); g.lineTo(3.4, 0.4); g.closePath(); g.fill(); g.stroke();
  ...
  set('grave_iron', (g, size, item) => {                          // a length of grave railing: two bars, a rail, a spear finial and eighty years of rust
    g.fillStyle = item.color;
    for (const [x, lean] of [[-4.4, -0.8], [3.4, 0.6]]) {                                                             // the two uprights, both bent
      g.beginPath(); g.moveTo(x - 1.6, -5.4); g.quadraticCurveTo(x - 1.6 + lean, 1.4, x - 1.2, 8.4);
      g.lineTo(x + 1.8, 8.4); g.quadraticCurveTo(x + 1.4 + lean, 1.4, x + 1.6, -5.4); g.closePath(); g.fill(); line(g, OUT, 1.2); g.stroke();
      g.beginPath(); g.moveTo(x - 1.6, -5.4); g.lineTo(x - 2.6, -6.6); g.lineTo(x, -9); g.lineTo(x + 2.6, -6.6); g.lineTo(x + 1.6, -5.4); g.closePath(); g.fill(); g.stroke(); }
    g.fillStyle = item.color;                                                                                         // the rail between them
    g.beginPath(); g.moveTo(-8.4, -1.4); g.lineTo(8.4, -2.6); g.lineTo(8.4, 0.4); g.lineTo(-8.4, 1.6); g.closePath(); g.fill(); line(g, OUT, 1.2); g.stroke();
    g.fillStyle = '#7a4a2a'; for (const [x, y] of [[-5, 4.4], [4.4, 5.4], [-1.4, -0.6], [6.4, -1.4]]) { g.beginPath(); g.arc(x, y, 1.2, 0, 7); g.fill(); }   // the rust pits
  });
  ...
  set('spider_silk', (g, size, item) => {                         // a spun bundle, wound tight, with the thread trailing off both ends
    g.save(); g.rotate(-0.45);
    g.fillStyle = item.color;
    g.beginPath(); g.ellipse(0, 0, 8, 4.2, 0, 0, 7); g.fill(); line(g, OUT, 1.2); g.stroke();
    line(g, dk(0.26), 1.2);                                                                                            // the winding, tighter at the ends
    for (const [x, r] of [[-5.4, 2.4], [-2.6, 3.6], [0.4, 4.1], [3.4, 3.4], [6, 2.2]]) { g.beginPath(); g.ellipse(x, 0, 1.4, r, 0, 0, 7); g.stroke(); }
    g.fillStyle = lt(0.45); g.beginPath(); g.ellipse(-2.4, -1.8, 3, 1.2, -0.15, 0, 7); g.fill();
    line(g, item.color, 1.2, 'round');                                                                                 // the two loose ends
    g.beginPath(); g.moveTo(7.8, -0.6); g.quadraticCurveTo(10.2, -2.4, 9.4, -4.4);
    g.moveTo(-7.8, 0.6); g.quadraticCurveTo(-10.2, 2.4, -9.4, 4.4); g.stroke();
    g.restore();
  });
  set('wool', (g, size, item) => {                                // a fleece: all curls, and a cord round the middle of it
    g.fillStyle = item.color;                                                                                          // the puffs that make the scalloped outline
    for (const [x, y, r] of [[-4.6, -3.4, 4], [1, -4.4, 4.2], [5.4, -1.4, 3.8], [4, 3.4, 3.6], [-1.4, 4.6, 3.8], [-5.6, 2.4, 3.6], [0, 0, 4.6]]) {
      g.beginPath(); g.arc(x, y, r, 0, 7); g.fill(); }
    line(g, dk(0.26), 1.2);                                                                                            // the curl of it, drawn as loose spirals
    for (const [x, y] of [[-3.4, -2.6], [2.4, -2.4], [-1.4, 2.4], [4.4, 1.4]]) { g.beginPath(); g.arc(x, y, 1.8, 0.4, 4.6); g.stroke(); }
    g.fillStyle = CORD; g.fillRect(-8.2, -1.1, 16.4, 2.2); line(g, dk(0.35), 1); g.strokeRect(-8.2, -1.1, 16.4, 2.2);   // the tie
    g.fillStyle = lt(0.4); g.beginPath(); g.arc(-3.4, -4.4, 1.4, 0, 7); g.fill();
  });
