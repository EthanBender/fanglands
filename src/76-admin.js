...
// ============================================================================
{
  ...
  const NIGHT_CRYPT = { x: 13, y: 70 };   // 35-night: the crypt door east of the last knight's grave (12,70, 02-world)
  ...
  function hideInput() { if (!dom || !dom.shown) return; dom.shown = false; dom.key = ''; try { dom.input.blur(); } catch (e) { } dom.input.style.display = 'none'; }
  function showInput(w) {
    const d = ensureInput(); if (!d) return;
    const k = [w.x, w.y, w.w, w.h, w.mode].join(',');
    if (d.key !== k) {
      d.key = k; const s = d.input.style;
      s.left = Math.round(w.x) + 'px'; s.top = Math.round(w.y) + 'px'; s.width = Math.round(w.w) + 'px'; s.height = Math.round(w.h) + 'px';
      d.input.inputMode = w.mode === 'qty' ? 'numeric' : 'text';
      d.input.placeholder = w.placeholder || '';
      d.input.value = w.mode === 'qty' ? S.qtyText : S.search;
    }
  ...
  function focusInput(mode) {
    ...
    const d = ensureInput(); if (!d) return;
    d.key = ''; d.input.value = mode === 'qty' ? S.qtyText : S.search;
  ...
  function open(tab) {
    ...
    S.view = null; S.inputMode = 'search';
  ...
  function drawPowers(g, x, y, w, h, T) {
    ...
    const entries = [
      ...
      { key: 'admin:give', text: 'Give me an item', color: '#21262d', enabled: true, action: () => { S.view = { kind: 'give' }; S.inputMode = 'search'; }, line: 'Anything in the game, as many as you like.' },
    ];
  ...
  function drawGive(g, x, y, w, h, T) {
    ...
    row(g, x, y, w, T, [
      { text: 'Back', w: 70, action: () => { S.view = null; S.inputMode = 'search'; }, color: '#21262d', key: 'admin:back' },
  ...
  HOOKS.update.push(dt => {
    ...
    spawnUpkeep();
    if (S.followUp > 0 && --S.followUp === 0) { seeAllMarkers(); save(); }
  });
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    try {
      ...
      let pinnedRaw = null;
      { refill(); closePanel(); dialog.cur = null; dialog.queue.length = 0; render(); save(); const before = slot();
        ...
        const onlyPresence = sent.every(m => m.t === 'p');
        check(P + `Unlock everything with a pin that answers: the pinned backup is the knight from before, and every entry of the table holds afterwards (${UNLOCKS.length}), the guild founded, and nothing but presence went out on the socket`, pinnedRaw === before && told && failing.length === 0 && founded && onlyPresence, { pinnedBefore: pinnedRaw === before, told, failing, founded, onlyPresence, out: sent.map(m => m.t), pinAt: S.pinAt });
