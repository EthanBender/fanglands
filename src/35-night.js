...
// ============================================================================
{
  ...
  function spawnTile() {
    ...
    for (let k = 0; k < 40; k++) {
      ...
      if (SOLID.has(t) || PUSH_THROUGH.has(t) || buildingAt(tx, ty) || inVillageBounds(tc(tx), tc(ty)) || isCaveTile(tx, ty)) continue;
  ...
  HOOKS.update.push(dt => {
    ...
    for (let i = monsters.length - 1; i >= 0; i--) {
      ...
      if (ph !== 'night') { crumble(m); monsters.splice(i, 1); }          // dawn
    }
  ...
  // ---------- test / debug handle ----------
  window.NIGHT = { DAY, LIGHT, DUSK, phase, alpha, overlay: overlayAlpha, dayT, alive: nightAlive, resetTimer: () => { spawnT = 0; }, timer: () => spawnT, canSpawn, tiles: { crypt: T_CRYPT, deadTree: T_DEAD } };
