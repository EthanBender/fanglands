...
//      one thing you can only get here — Hux's pick, tier 3, the best pickaxe in the Fanglands.
...
// ============================================================================
{
  ...
  RECIPES.push({ out: 'salt_beef', qty: 3, needs: [['raw_beef', 3], ['redsalt', 2]], station: 'workbench',
  ...
  const inPl = (x, y) => ((x - CX) / RX) ** 2 + ((y - CY) / RY) ** 2 <= 1;
  ...
  function classOf(x, y) {
    if (!inPl(x, y)) return canD(x, y) <= 3 ? 0 : 4;
    const d = canD(x, y);
    if (d <= 3) return 0;
    if (d <= 4.2) return 1;
    if (d <= 6.2) return 2;
  ...
  HOOKS.world.push((rnd0, api) => {
    ...
    // 1. the ground: four heights over the table, dry scrub everywhere outside it
    ...
    for (let y = FALL.y0; y <= FALL.y1; y++) for (let x = FALL.x0; x <= FALL.x1; x++) {
      const c = classOf(x, y); if (c === 3 || c === 4) continue;
    ...
    for (const [x, y] of BOARDS) { force(x, y, RC_BOARD); for (const [dx, dy] of [[0, 1], [1, 0], [-1, 0], [0, -1]]) if (SOLID.has(at(x + dx, y + dy))) force(x + dx, y + dy, classOf(x + dx, y + dy) === 4 ? RC_SCRUB : RC_TOP); }
    ...
    //     back to cliff unless it is one of the ways in that were cut on purpose.
    {
      ...
      let sealed = 0;
      for (let y = RC.y0; y <= RC.y1; y++) for (let x = RC.x0; x <= RC.x1; x++) {
        if (classOf(x, y) !== 2 || opening.has(x + ',' + y)) continue;
    ...
    for (let y = RC.y0; y <= RC.y1; y++) for (let x = RC.x0; x <= RC.x1; x++) {
      const t = at(x, y);
      if (t !== RC_SCRUB && t !== RC_TOP && t !== RC_FLOOR) continue;
  ...
  const paint = { grounds: 0, faces: 0, rims: 0, shadows: 0, props: 0 };
  const TONE = {
    ...
    [RC_LAMP]: ['#8a4c30', '#965436', '#80452b'], [RC_BOARD]: ['#9c8654', '#a48e5c', '#947e4d'],
    [RC_BUSH]: ['#9c8654', '#a48e5c', '#947e4d'],
  };
  ...
  HOOKS.draw.push((g, items) => {
    const x0 = Math.max(RC.x0, Math.floor(cam.x / TILE) - 1), x1 = Math.min(RC.x1, Math.ceil((cam.x + VW) / TILE) + 1);
    const y0 = Math.max(RC.y0, Math.floor(cam.y / TILE) - 1), y1 = Math.min(RC.y1, Math.ceil((cam.y + VH) / TILE) + 1);
    if (x1 < x0 || y1 < y0) return;
    items.push({ y: -8e8, draw: () => {
      for (let ty = y0; ty <= y1; ty++) for (let tx = x0; tx <= x1; tx++) {
        const t = tileAt(tx, ty), tone = TONE[t];
  ...
  function drawLamp(g, tx, ty) {
    const cx = tc(tx), cy = tc(ty), lit = !(quest.redcut && quest.redcut.stage >= 2);
  ...
  HOOKS.draw.push((g, items) => {
    const x0 = Math.max(RC.x0, Math.floor(cam.x / TILE) - 1), x1 = Math.min(RC.x1, Math.ceil((cam.x + VW) / TILE) + 1);
    const y0 = Math.max(RC.y0, Math.floor(cam.y / TILE) - 1), y1 = Math.min(RC.y1, Math.ceil((cam.y + VH) / TILE) + 2);
    if (x1 < x0 || y1 < y0) return;
    for (let ty = y0; ty <= y1; ty++) for (let tx = x0; tx <= x1; tx++) {
  ...
  HOOKS.talk.redcut_rim = n => {
    ...
    if (q.stage === 0) {
      ...
      say('My knee will not take the switchback, and I will not leave the winch. You take the ledges — three of them, just north of here — and you go down and you find him. Take the end with you. He knows his own knot.', n.name);
  ...
  if (HOOKS.xpSource) HOOKS.xpSource.push(add => {
    ...
    add('cooking', 'salt beef at a workbench (3 beef + 2 redsalt → 3)', 15, 90, 12, 'the redsalt is mined in the Redcut');
  });
  ...
  if (window.WIKI) WIKI.add('places', { id: 'redcut', name: 'The Redcut', lines: [
    ...
    'WHAT IS IN IT. Redsalt, Mining 22, from the seams in the canyon walls — it is mined nowhere else. Three redsalt and three raw beef at any workbench make three salt beef, which heals 13 and cannot burn.',
    'THE DEEP CACHE, behind the crack at the foot of the box head. Agility 30 to squeeze through. Hux\'s pick is in it: the best pickaxe in the Fanglands.',
  ...
  window.REDCUT = { RC, CX, CY, RX, RY, ARC, SPUR, MOUTH, SLOT, RISE, STAIR, SWITCHBACK, FALL, WINCH_T, BASKET_T, LAMP_T, COIL_T, CRACK_T, CACHE_T, BENCH_T, MARLOW, HUX, NESTS, BOARDS, HAWKS, JAWS, SEAMS: REDCUT_SEAMS,
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    { const c = {}; for (let y = RC.y0; y <= RC.y1; y++) for (let x = RC.x0; x <= RC.x1; x++) { const n = tileName(tileAt(x, y)); c[n] = (c[n] || 0) + 1; }
      ...
      check(P + 'The Redcut is a region in the bottom-right, and the table is cut into four heights: canyon floor, ledge, cliff and table top',
        ...
        && (c.CANYONFLOOR || 0) > 350 && (c.REDLEDGE || 0) > 120 && (c.REDCLIFF || 0) > 500 && (c.REDROCK || 0) > 900 && (c.REDSCRUB || 0) > 700,
        { region: reg && [reg.x0, reg.y0, reg.x1, reg.y1], floor: c.CANYONFLOOR || 0, ledge: c.REDLEDGE || 0, cliff: c.REDCLIFF || 0, top: c.REDROCK || 0, scrub: c.REDSCRUB || 0, tally }); }
    ...
    { const p0 = { ...paint }; const px = player.x, py = player.y;
      ...
      // and out on the open scrub there is no cliff to draw
      const p1 = { ...paint }; F.tp(210, 101); render();
    ...
    { let inside = 0, outside = 0;
      ...
      check(P + 'redsalt is cut from seams in the Redcut and nowhere else on the map: Mining 22, 82 xp, and 3 beef + 2 redsalt make 3 salt beef that cannot burn',
        inside >= 8 && outside === 0 && !!g && g.lv === SALT_LV && g.xp === 82 && g.item === 'redsalt' && !!rec && rec.qty === 3 && rec.skill === 'cooking' && ITEMS.salt_beef.heal === 13 && !ITEMS.salt_beef.cook && refused,
        { seamsInside: inside, seamsElsewhere: outside, mineLv: g && g.lv, refusedBelowLevel: refused, seam, side, notice: notice && notice.text, heal: ITEMS.salt_beef.heal }); }

    // 8. Hux's pick: tier 3, the best pickaxe there is, and it is in the cache and nowhere else
    { const picks = Object.keys(ITEMS).filter(k => ITEMS[k].tool === 'pickaxe');
      const best = Math.max(...picks.map(k => ITEMS[k].tier || 0));
      ...
      check(P + "Hux's pick is tier 3, the best pickaxe in the game, and the Deep Cache is the only place it comes from", ITEMS.hux_pick.tier === 3 && best === 3 && !inShops && !inDrops && !inRecipes && tileAt(CACHE_T.x, CACHE_T.y) === RC_CACHE,
        { tier: ITEMS.hux_pick.tier, bestPickaxeTier: best, inShops, inDrops, inRecipes }); }
    ...
    { const q0 = JSON.parse(JSON.stringify(Q())), bag = player.inv.slice();
      ...
      clearSay(); player.inv = bag; quest.redcut = q0;
      check(P + 'The Cut Rope runs start to finish: Marlow accuses the rim, the lamp is still lit, Hux says nothing until you have looked at the coiled rope, and then he says he cut it himself',
        started && winchDead && lamp && heldBack && clue && turn && done && wentDown && cameUp,
