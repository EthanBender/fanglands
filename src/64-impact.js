...
// stomp (22-bulldozer), Full Steam and the walker's Quake Stomp (55-riding), The Gnasher's arm and its fall (33-goblincity), and The Fang
...
// ============================================================================
{
  ...
  // At most six rings in the air at once. When all six are still moving, a new hit makes no ring rather than
  // pushing the oldest one out: a ring cut off part way through would drop every tile it had lifted back flat in
  // one frame, and a tile must always come back to where it was by settling, never by a snap.
  const MAX_WAVES = 6;
  ...
  const waves = [];
  let last = null, drawn = 0, dropped = 0;
  ...
  function wave(x, y, strength, tag) {
    ...
    const w = { cx: x / TILE, cy: y / TILE, r: radiusTiles(s), lift: liftPx(s), t0: time, s, tag: tag || '', inst: window.__instance || null }; // the map it was made on
    // a ring that has finished settling frees its slot; one still lifting ground keeps it
    if (waves.length >= MAX_WAVES) prune();
    if (waves.length >= MAX_WAVES) { dropped++; return null; }
    waves.push(w); last = w;
    return w;
  }
  ...
  // A wave belongs to the map it was made on: walk through a door and the ground you left stops moving.
  // (This used to compare the live instance against one remembered on the last update tick, and cleared EVERY
  // wave when they differed — so a wave made right after entering or leaving a map, before any tick had run,
  // was wiped on the first frame it should have moved. The self-test's ring was gone after one tick.)
  HOOKS.update.push(() => {
    if (!waves.length) return;
    const i = window.__instance || null;
    for (let k = waves.length - 1; k >= 0; k--) if (waves[k].inst !== i) waves.splice(k, 1);
    if (!waves.length) { last = null; drawn = 0; }
  });

  HOOKS.newGame.push(() => { clearWaves(); ramsWas = -1; steamWas = null; fangRef = null; fangAlive = false; fangEl = null; });
  ...
  window.IMPACT = {
    ...
    get dropped() { return dropped; }, // hits that made no ring because six were already moving
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    try {
      ...
      // ---------- M. the walker's Quake Stomp: one ring, and it cuts no other ring short ----------
      // It used to call wave() on every frame of the 1.15 s stomp with no tag: 69 waves, each one pushing the oldest
      // out of the six-wave list after about six frames, so no ring ever got past the tile under the machine and
      // the boiler's ring was thrown away 0.1 s in. Driven through the real game step so the clock moves.
      { clearWaves();
        let quakeOk = false, quakeInfo = { hasRiding: !!window.RIDING };
        if (window.RIDING) {
          const q = h.openSpot(58, 36); F.tp(q.x, q.y);
          player.companion = null;
          // a machine with no kind is the walker
          player.mech = { hp: 110, maxHp: 110 }; player.r = 22; player.facing = { x: 1, y: 0 };
          window.RIDING.resetCool(); steamWas = null;
          // the stomp hurts and stuns what is near: this check is about the ground, so nothing nearby is touched
          const near = monsters.filter(m => !m.dead && dist(m.x, m.y, player.x, player.y) < 400).map(m => ({ m, x: m.x, y: m.y, stunT: m.stunT, state: m.state, angry: m.angry }));
          const _hitMonster = hitMonster; hitMonster = function () { };
          const cx = Math.floor(player.x / TILE), cy = Math.floor(player.y / TILE);
          const d0 = dropped, made = new Set(), cut = [], far = {};
          let other = null, frames = 0, kind = null;
          try {
            kind = window.RIDING.machineKind();
            window.RIDING.startSteam();
            while (frames < 300 && (window.RIDING.special || waves.length)) {
              F.step([]); frames++;
              const sp = window.RIDING.special, running = !!sp && sp.phase === 'run';
              // another ring already in the air when the feet come down (a bomb, say): it must live out its whole wave
              if (running && !other) other = wave(player.x, player.y, 1, 'other-ring');
              for (const w of made) if (!waves.includes(w) && !w.gone) { w.gone = true; if (time - w.t0 < LIFE) cut.push({ tag: w.tag, age: +(time - w.t0).toFixed(3) }); }
              for (const w of waves) made.add(w);
              // while the stomp runs, the highest lift at each whole-tile distance from the machine
              if (running) box(cx, cy, 7, (tx, ty) => { const d = Math.round(Math.hypot(tx - cx, ty - cy)); const l = geomLift(tx, ty); if (l > (far[d] || 0)) far[d] = +l.toFixed(2); });
            }
          } finally {
            hitMonster = _hitMonster;
            for (const s of near) Object.assign(s.m, { x: s.x, y: s.y, stunT: s.stunT, state: s.state, angry: s.angry });
            window.RIDING.resetCool();
          }
          const tags = [...made].map(w => w.tag);
          const quakes = tags.filter(t => t === 'quake').length;
          // the boiler's ring (1, radius 3) and the other ring (1, radius 3) lift nothing 3 or more tiles out: any lift
          // there while the stomp runs is the Quake's own ring
          const reach = Math.max(0, ...Object.keys(far).filter(d => +d >= 3).map(d => far[d]));
          quakeOk = kind === 'walker' && quakes === 1 && made.size <= 4 && cut.length === 0 && dropped === d0 && !!other && reach > 1 && (far[5] || 0) > 0;
          quakeInfo = { hasRiding: true, kind, waves: made.size, tags, quakes, cut: cut.length, firstCut: cut[0], dropped: dropped - d0, liftByTiles: far, frames };
        }
        player.mech = mech0; player.dozerUp = up0; player.r = r0; player.speed = sp0; player.companion = comp0; player.attackT = 0; steamWas = null; clearWaves();
        check(P + "the walker's Quake Stomp sends one ring out across the ground and cuts no other ring short",
          quakeOk, quakeInfo); }

      // ---------- N. a full list turns the new hit away rather than cutting a ring off mid-lift ----------
      { clearWaves();
        const res = withTime(() => {
          const d0 = dropped;
          const six = [0, 1, 2, 3, 4, 5].map(k => wave(tc(o.x), tc(o.y), 3, 'full-' + k));
          // a tenth of a second on, all six are still lifting
          time += 0.1;
          const lifting = geomLift(o.x + 1, o.y) > 0;
          const seventh = wave(tc(o.x), tc(o.y), 3, 'full-6');
          const kept = waves.length === MAX_WAVES && six.every(w => w && waves.includes(w));
          const turnedAway = seventh === null && dropped - d0 === 1 && last === six[5];
          // the first one has settled: its slot is free now, with no update tick in between
          six[0].t0 = time - LIFE;
          const eighth = wave(tc(o.x), tc(o.y), 3, 'full-7');
          const reused = !!eighth && waves.length === MAX_WAVES && !waves.includes(six[0]) && six.slice(1).every(w => waves.includes(w)) && dropped - d0 === 1;
          return { lifting, kept, turnedAway, reused, dropped: dropped - d0 };
        });
        clearWaves();
        check(P + 'with six rings already moving a seventh hit makes no ring instead of cutting one off mid-lift, and a ring that has settled frees its slot at once',
          res.lifting && res.kept && res.turnedAway && res.reused, res); }
    } finally {
      clearWaves();
