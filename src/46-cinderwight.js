...
// ============================================================================
{
  ...
  const AFTER_ANCHORS = [[16, 12], [42, 32]];   // inside the Afterlands (60 x 40): west of the chapel, south-east of the graveyard
  ...
  HOOKS.hud.push(g => {
    ...
    const w = Math.min(236, VW - 28), h = feeding ? 60 : 48, x = 14;
    const lay = typeof HUD_LAYOUT !== 'undefined' ? HUD_LAYOUT : null;
    const touchFloor = (isTouch && lay && !lay.short) ? lay.hotbarY + lay.hotbarH + 12 : 0;
    const y = Math.max(HUD.leftY, touchFloor, 84);   // shared left-HUD cursor: stack under whatever drew above
    HUD.leftY = y + h + 6;
    ...
    if (feeding && heart) {
      g.fillStyle = '#2a2f3a'; roundRect(g, x + 12, y + 42, w - 24, 9, 4); g.fill();
      g.fillStyle = col; roundRect(g, x + 12, y + 42, (w - 24) * clamp(heart.hp / heart.maxHp, 0, 1), 9, 4); g.fill();
      g.fillStyle = '#fff'; g.font = 'bold 9px sans-serif'; g.textAlign = 'center'; g.fillText(`heart ${Math.ceil(heart.hp)} / ${heart.maxHp}`, x + w / 2, y + 50);
    }
  });
  ...
  window.CINDERWIGHT = {
    WIGHT, HEART, ASH_SPOTS, AFTER_SPOTS, heartOf, kindle,
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    { const inst = window.INSTANCES && typeof INSTANCES.get === 'function' ? INSTANCES.get('afterlands') : null;
      ...
      let entered = false, inside = 0, walked = null, left = false, region = null;
      if (inst && !player.mech && !window.__instance) {
        ...
        if (entered) {
          ...
          if (there.length) { const t = there.map(homeT).sort((a, b) => a.x - b.x || a.y - b.y)[0]; walked = F.walkTo(t.x, t.y, 8000); }
