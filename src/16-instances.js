...
// ============================================================================
{
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    { enterByDoor(); const bm = monsters.find(m => m.type === 'brood_mother'); F.tp(DEN_BOSS[0] + 3, DEN_BOSS[1]); bm.x = tc(DEN_BOSS[0]); bm.y = tc(DEN_BOSS[1]); bm.state = 'idle'; bm.wanderT = 99; bm.wander = { x: 0, y: 0 }; bm.eggT = 0;
      ...
      bm.eggT = 0; F.sim(2, []); const laid2 = eggs(); const e = active.eggs[0]; let swung = false, byE = false, live = null;
      ...
      // a swing sweeps its reach on every tick of its 0.22 s (attackT), so an egg laid or stood beside before it ends breaks to the swing, not to E: let it end first
      for (let i = 0; i < 60 && player.attackT > 0; i++) F.step([]);
      let e2 = active.eggs[0]; if (!e2) { bm.eggT = 0; F.sim(2, []); e2 = active.eggs[0]; }
      if (e2) { const sp = beside(e2.tx, e2.ty); player.x = sp.x; player.y = sp.y; F.face(e2.tx, e2.ty); closePanel(); live = player.attackT; F.press('KeyE'); F.sim(2, []); byE = live === 0 && tileAt(e2.tx, e2.ty) === T.CAVE && !active.eggs.includes(e2); }
      check('instance: the Brood Mother lays 2 eggs that hatch into spiders after 6 s; a swing or E breaks one', laid === 2 && hatched && laid2 === 2 && swung && byE, { laid, hatched, laid2, swung, byE, swingLeftAtE: live, eggsNow: eggs() });
