...
// ============================================================================
{
  ...
  HOOKS.use.push((t, tx, ty) => {
    ...
    if (t === CAN_RAIL) { notify('A rail of woven vine. The run goes round it, not through it.'); return true; }
    if (t === ROOF_RIDGE) { notify('The roof ridge. Scrap iron over a beam. The run goes round it.'); return true; }
  ...
  window.AGILITY2 = { CANOPY, ROOFTOP, CAN, ROOF, VINE, SCAFFOLD, CANOPY_LV, VINE_LV, ROOF_LV, SCAF_LV, TILES: { CAN_DECK, CAN_RAIL, CAN_VINE, ROOF_DECK, ROOF_RIDGE, GC_SCAFFOLD } };
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    // a BFS the knight could actually walk: solid stops it, doors and gates do not, and `open` lists the shortcut
    // tiles to treat as climbable. Returns the number of steps, or -1 when there is no way at all.
    const walkLen = (sx, sy, tx, ty, open = []) => {
      const W = MAP_W, H = MAP_H, prev = new Int32Array(W * H).fill(-1), q = [sy * W + sx];
      prev[sy * W + sx] = sy * W + sx;
      const pass = t => open.includes(t) || !SOLID.has(t) || PUSH_THROUGH.has(t);
      ...
      if (prev[ty * W + tx] === -1) return -1;
      let len = 0, c = ty * W + tx; while (c !== sy * W + sx) { len++; c = prev[c]; }
      return len;
    };
    // walk one full lap by touching the flags in order, the way 38-agility's yard check does
    const lap = (course, marks) => {
      const st = AGI.state(); st.next[course] = 0; const laps0 = st.laps[course], xp0 = player.skills.agility.xp, c0 = coins();
      for (const i of [0, 1, 2, 3, 4, 0]) { const [x, y] = marks[i]; F.tp(x, y); F.step([]); }
    ...
    { setLv(CANOPY_LV); h.give('coins', 1);
      const st = AGI.state(); st.laps.canopy = CANOPY.bonusEvery - 1;
      const r = lap('canopy', CAN_MARKS);
      const st2 = AGI.state(); st2.next.canopy = 0; const bad = lap('canopy', [CAN_MARKS[0], CAN_MARKS[2], CAN_MARKS[1], CAN_MARKS[3], CAN_MARKS[4], CAN_MARKS[0]].slice(0, 5));
      check(P + `a canopy lap pays ${CANOPY.xp} Agility xp and ${CANOPY.bonus} coins every ${CANOPY.bonusEvery} laps; the flags out of order pay nothing`,
        r.xp === CANOPY.xp && r.laps === 1 && r.coins === CANOPY.bonus && bad.xp === 0 && bad.laps === 0,
        { lapXp: r.xp, laps: r.laps, bonus: r.coins, outOfOrder: bad.xp, name: CANOPY.name }); }
    ...
    { const deck = ROOF_TRACK.filter(([x, y]) => [ROOF_DECK, AG_LOG, AG_NET, AG_GAP, AG_MARK].includes(tileAt(x, y))).length;
      ...
      setLv(ROOF_LV); F.tp(ROOF.x0, ROOF.north - 1); const walked = F.walkTo(ROOF_MARKS[2][0], ROOF_MARKS[2][1], 3000);
      check(P + 'the Grubmarket rooftop run is carved along the market roofs (panels, ridge, logs, nets, two gaps, five flags) and you can walk to it from the ferry landing',
        deck === ROOF_TRACK.length && ridge === ROOF.x1 - ROOF.x0 - 1 && pieces && !!reach && typeof walked === 'number' && tileOf().tx === ROOF_MARKS[2][0],
        { deck, track: ROOF_TRACK.length, ridge, pieces, reach: reach && reach.length, walked, at: tileOf() }); }
    ...
    { setLv(ROOF_LV - 1); const st = AGI.state(); st.next.rooftop = 4; floaters.length = 0;
      ...
      check(P + `the rooftop run's alley jump fails informatively below Agility ${ROOF_LV} (2 damage, back to the start flag, lap lost) and clears at ${ROOF_LV}`,
        fell && backAtStart && jumped, { fell, backAtStart, jumped, hp: player.hp, maxHp: player.maxHp }); }
    { setLv(ROOF_LV); h.give('coins', 1);
      const st = AGI.state(); st.laps.rooftop = ROOFTOP.bonusEvery - 1;
      const r = lap('rooftop', ROOF_MARKS);
      const st2 = AGI.state(); st2.next.rooftop = 0; const bad = lap('rooftop', [ROOF_MARKS[0], ROOF_MARKS[2], ROOF_MARKS[1], ROOF_MARKS[3], ROOF_MARKS[4]]);
      check(P + `a rooftop lap pays ${ROOFTOP.xp} Agility xp and ${ROOFTOP.bonus} coins every ${ROOFTOP.bonusEvery} laps; the flags out of order pay nothing`,
        r.xp === ROOFTOP.xp && r.laps === 1 && r.coins === ROOFTOP.bonus && bad.xp === 0 && bad.laps === 0,
        { lapXp: r.xp, laps: r.laps, bonus: r.coins, outOfOrder: bad.xp, name: ROOFTOP.name }); }
