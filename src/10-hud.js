...
// Rows a feature adds to the foot of the pack, one kit row each, full width: { id, show?: () => bool, draw: (g, x, y, w, h) }.
// 69-retaliate puts its Fight back switch here. The pack measures them in, so they never land on the grid or the verbs.
const PACK_ROWS = [];
function drawPackPanel(g) {
  ...
  const extras = PACK_ROWS.filter(e => !e.show || e.show());
  // an upright phone has no height to spare: one feature row rides on the keyring's line when the tiles leave it room
  const keysW = keys.length * (44 + G) - G, inlineX = stacked && keys.length && extras.length === 1 && extras[0].minW ? (Rm.aw - 36) - keysW - 16 - extras[0].minW(g, R) >= 0 : false;
  const extraH = extras.length && !inlineX ? extras.length * (R + G) + 4 : 0;
  ...
  if (stacked) {
    ...
    const measure = () => { const D = dims(); return 62 + HEAD + D.gridH + 12 + Math.max(HEAD + D.wornH, sideH) + 18 + itemH + 8 + R + 12 + keyRow + extraH + 8; };
    ...
    const belowH = 62 + band + 18 + Math.max(itemH, R) + 12 + Math.max(keyRow, !side && home ? R : 0) + extraH + 12;
    if (belowH <= Rm.ah && belowW <= Rm.aw - 36) { mode = 'below'; cw = belowW; h = belowH; }
      ...
      h = 62 + Math.max(HEAD + D.gridH + 8 + statsH, infoH) + extraH + 12;
    }
  }
  const { gridW, gridH, wornW, wornH, aW } = dims();
  const nUsed = player.inv.filter(Boolean).length;
  const { px, py, w, h: ph } = panelBox(g, cw + 36, h, 'Your pack', `${nUsed} of ${INV_SLOTS} pouches used. Tap two to swap them.`);
  ...
  if (inlineX) { const e = extras[0], ex = ix + keysW + 16; e.draw(g, ex, ky + 18, ix + iw - ex, R); }
  if (homeHere) {
    if (mode === 'below') packHome(g, ix + iw - 150, ky + (keys.length ? HEAD : 0), 150, R);
    else packHome(g, ix, ky + keyRow, Math.min(iw, 160), R);
  }
  // ---- the feature rows (PACK_ROWS) along the foot, under a hairline ----
  if (extras.length && !inlineX) {
    let ey = py + (ph || h) - 12 - extras.length * (R + G) + G;
    K.hair(g, x0, ey - 6, cwR, 0.3);
    for (const e of extras) { e.draw(g, x0, ey, cwR, R); ey += R + G; }
  }
}
...
function helpRows() {
  ...
  if (t) return [
    ...
    { em: 'block', name: 'BAG, Fight back', text: 'On: a monster that hits you gets hit back.' },
...
function drawMapPanel(g, narrow) {
  // Inside an instance the map is that instance (09-render mapView): its name on the panel, its own rect drawn whole and
  // centred, and none of the overworld's region names, home or quest rings (they sit in the same top-left tiles).
  const K = PANEL_KIT, Rm = K.room(), T = HK.T;
  const view = mapView(), inside = !!view.id;
  let mw = Math.min(Rm.aw, 760); if (inside) mw = Math.min(mw, Math.max(360, (Rm.ah - 80) * view.w / view.h + 36));
  const mh = Math.min(Rm.ah, (mw - 36) * view.h / view.w + 80);
  const close = touchMode() ? 'Tap the map to close.' : 'Click the map to close.';
  const { px, py, w, h } = panelBox(g, mw, mh, view.name, inside ? `${view.sub ? view.sub + '. ' : ''}White arrow: you. ${close}` : `White arrow: you. Blue: home. Gold rings: quests. ${close}`);
  if (miniDirty || miniDiffCount !== mapDiffs.size) refreshMini();
  const iw = w - 36, ih = Math.min(h - 80, iw * view.h / view.w), ix = px + 18, iy = py + 62;
  // the world keeps its fit (full width, cropped at the foot on a short screen); an instance fits whole and is centred
  ...
  {
    const ax = ox + player.x / TILE * sc, ay = oy + player.y / TILE * sc, ang = Math.atan2(player.facing.y || 0, player.facing.x || 1);
...
function drawCoffinPanel(g) {
  // The chest holds everything from every fall until it is collected (06-systems deathKeep), so it can be long: the
  // total in plain words on top, every entry listed a page at a time with its own fee (or "no room" when the pack is
  // full), and the reclaim plate charges only for what fits (reclaimPlan).
  const K = PANEL_KIT, Rm = K.room(), T = HK.T, { R } = Rm;
  const W = Math.min(Rm.aw, 480), cw = W - 36, lhI = 26;
  let words, plan = null, fee = 0, rest = 0;
  if (!deathKeep) words = 'The chest is empty. For now. When you fall, what you carry comes here, and it stays here, from every fall, until you collect it. Cheap things I return for nothing. Precious things cost a share of their worth, and I take it from your own coin first.';
  else {
    ...
    words = 'I keep everything from every fall until you collect it. ' + (plan.all
  ...
  const items = deathKeep ? deathKeep.items : [], n = items.length;
  // rows between the heading and the reclaim plate; a pager row when they do not all fit
  const fixed = 62 + 20 + wh + 10 + 22 + 12 + R + 16, pagerH = R + 10;
  let per = Math.max(1, Math.floor((Rm.ah - fixed) / lhI));
  if (n > per) per = Math.max(1, Math.floor((Rm.ah - fixed - pagerH) / lhI));
  const pages = Math.max(1, Math.ceil(n / per)); coffinPage = clamp(coffinPage, 0, pages - 1);
  const shownN = Math.min(per, n);
  const h = deathKeep ? fixed + shownN * lhI + (pages > 1 ? pagerH : 0) : 62 + 20 + wh + 16;
  ...
  K.head(g, x0, y + 16, cw, `IN THE CHEST: ${chestWords().toUpperCase()}`);
  y += 22;
  const first = coffinPage * per;
  items.slice(first, first + per).forEach((sl, k) => {
    const i = first + k, coin = sl.id === 'coins', fits = plan.fits[i], f1 = plan.fees[i];
    try { drawItemIcon(g, sl.id, x0 + 10, y + lhI / 2, 16); } catch (e) { }
    const feeT = coin ? 'free' : fits <= 0 ? 'no room' : f1 > 0 ? `${f1} coins` : 'free', fw = HK.tw(g, feeT, K.FN(12)) + 6;
    const nm = `${ITEMS[sl.id] ? ITEMS[sl.id].name : sl.id} × ${sl.qty}` + (!coin && fits > 0 && fits < sl.qty ? ` (room for ${fits})` : '');
    let t = nm; const room = cw - 26 - fw - 8, f = K.FB(13); if (HK.tw(g, t, f) > room) t = (HK.wrap(g, t, room, 1, f).lines[0] || t);
    HK.text(g, t, x0 + 26, y + lhI / 2 + 5, { font: f, color: T.ink, box: { x: x0 + 26, y, w: room, h: lhI }, fitId: 'coffin:item' });
    K.name(g, feeT, x0 + cw, y + lhI / 2 + 5, fw, { size: 12, align: 'right', color: !coin && fits <= 0 ? T.warn : !coin && f1 > 0 ? T.gold : T.inkMute });
    K.hair(g, x0, y + lhI - 1, cw, 0.18);
    y += lhI;
  });
  if (pages > 1) pager(g, x0, py + ph - 16 - R - 10 - R, cw, coffinPage, pages, q => { coffinPage = clamp(q, 0, pages - 1); });
  const ok = coins() >= rest;
  ...
  K.verb(g, x0, py + ph - 16 - R, cw, R, label, reclaimFromDeath, { emblem: 'coin', tone: 'primary', enabled: ok, name: ok ? 'Take your things back' : `You need ${rest} coins` });
}
