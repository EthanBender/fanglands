...
// ============================================================================
{
  ...
  window.BANK = { QTIES, KIND_ORDER, TAB_LABEL, resetState, qty, setQty, mode, setMode, sortOn, setSort, setTab, tabNow, tabsNow, kindOf, viewFor, wantOf, layout, depositItem, depositFromPack, withdraw, wearFromVault, bankWorn, depositBag, depositWorn, inVault, eqSlotFor };
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    try {
      ...
      { clear(); const ids = Object.keys(ITEMS);
        ...
        check(P + 'all ' + ids.length + ' items in the game file themselves into exactly one tab, and the All tab holds every one',
      ...
      // an empty tab is never offered, and the tab you were on falls back to All when it empties
      { clear(); player.bank = [{ id: 'steel_sword', qty: 1 }, { id: 'shark', qty: 5 }]; open();
        const two = BANK.tabsNow().join(',') === 'all,gear,food' && !buttons.some(b => b.label === 'tab:quest') && !buttons.some(b => b.label === 'tab:craft');
        F.clickButton('tab:gear');
        const onGear = BANK.tabNow() === 'gear';
        F.clickButton('bank' + vaultIndexOf('steel_sword'));       // the last piece of gear leaves the vault
        const gone = BANK.tabsNow().join(',') === 'all,food' && !buttons.some(b => b.label === 'tab:gear');
        const fellBack = BANK.tabNow() === 'all' && shown().join(',') === 'shark';
        clear(); open();
        const empty = BANK.tabsNow().join(',') === 'all' && buttons.some(b => b.label === 'tab:all');
        check(P + 'a tab with nothing in it is never shown, and emptying the tab you are on drops you back to All', two && onGear && gone && fellBack && empty, { two, onGear, gone, fellBack, empty, tabs: BANK.tabsNow() }); }
