...
//
// STREETS (review round 1): the paving is two things now. The eleven named streets of PLAN.ROADS (the Kingsway from the
// Great Gate to the Flight Gate, the Ring Road inside the wall, Forge Street, the Garden Walk, Crown Street, Guild Row,
// Span Street, Market Way, Inn Lane, and on the Crown the Crown Walk and the Rookery Yard) are grey-blue setts with a
// white kerb and a gold gutter on every edge; the paving between them is pale flagstones. The Crown, once bare cloud,
// is a garden with a low white-and-gold wall round its edge (K3b, K3c).
//
...
// ============================================================================
{
  ...
  function queenFirst(e) {
    ...
    if (s === 6) {
      ...
      say('She gave you her first feather. Keep it. The gold updraft stones know it now. Walk up to one and use it, and Lark will carry you anywhere in the city.', who);
  ...
  function sceneFinish() {
    ...
    say('Here. This is my first feather. You get one when you fly for the first time, and you give it to someone who helped. Walk up to a gold updraft stone and use it, and I will come.', 'Lark');
  ...
  HOOKS.questText.lark = () => {
    const s = Q().stage;
    if (s === 'done') return "Done. Lark's first feather is on your keyring. Walk up to a gold updraft stone in Aerie and use it (E, or tap it), and Lark will fly you anywhere in the city.";
    return {
  ...
  HOOKS.mapTarget.push(() => {
    ...
    const s = Q().stage;
    // map: 'aerie' marks the target as being on the instance's own map, so the instance map shows it (13-ux mapTargets)
    if (inside()) { const [p, label] = TARGETS[s]; return { x: p[0], y: p[1], label, id: 'lark', map: 'aerie' }; }
  ...
  { const _tapLabelFor = tapLabelFor;
    tapLabelFor = function (p) {
      if (p && inside() && (p.kind === 'use' || p.kind === 'walk' || p.kind === 'wall')) {
        ...
        if (t === PAVE) { const r = PLAN.roadAt(p.tx, p.ty); if (r) return r.name.replace(/^The /, ''); }
  ...
  // =========================================================================
  // 9. drawing
  //   Layers (y): the ground, blitted from cached chunks at -1e8 - 2, under 36's sky and cloud rows (they cover only cells
  //   that are theirs) and 88's deck (none where this file paints ground); the sky decor (cloud-tops in the moat and under the bridges, the plaza's
  ...
  // G_ROAD is a street (PLAN.ROADS: grey-blue setts, a white kerb and a gold gutter on every edge); G_PAVE is the paving
  // between the streets (pavements, squares, yards: big pale flagstones), so a street never runs into its surroundings
  const G_NONE = 0, G_PAVE = 1, G_PLAZA = 2, G_LAWN = 3, G_BLOOM = 4, G_MARBLE = 5, G_CWALL = 6, G_HWALL = 7, G_RUG = 8, G_DOOR = 9, G_BRIDGE = 10, G_CLOUD = 11, G_UNDER = 12, G_ROAD = 13;
  ...
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
    ...
    else if (OUR_PROPS.includes(c)) code = PLAN.roadAt(x, y) ? G_ROAD : codeFor(PLAN.groundAt(x, y), x, y);
    else if (THEIR_PROPS.includes(c)) { const gc = PLAN.groundAt(x, y); if (gc === '=' || gc === '"' || gc === '_') code = PLAN.roadAt(x, y) ? G_ROAD : codeFor(gc, x, y); }
    GROUND[i] = code;
  }
  ...
  // the paving between the streets: big square flagstones, cool white with blue-grey joints (the walls are cream brick)
  function patPave(g, v) {
    const r = mulberry32(0x9a5e + v * 131);
    g.fillStyle = '#b7c0ce'; g.fillRect(0, 0, 48, 48);
    const cols = ['#eef0f4', '#e6eaf0', '#f2f3f5', '#e3e7ee'];
    for (let j = 0; j < 2; j++) for (let i = 0; i < 2; i++) {
      const x = i * 24, y = j * 24;
      g.fillStyle = cols[Math.floor(r() * cols.length)]; g.fillRect(x + 1, y + 1, 22, 22);
      g.fillStyle = 'rgba(255,255,255,0.7)'; g.fillRect(x + 1, y + 1, 22, 1.2); g.fillRect(x + 1, y + 1, 1.2, 22);
      g.fillStyle = 'rgba(90,105,130,0.12)'; g.fillRect(x + 1, y + 21.6, 22, 1.4);
    }
    g.strokeStyle = 'rgba(140,155,180,0.18)'; g.lineWidth = 0.7; g.beginPath(); g.moveTo(4 + v * 9, 30); g.quadraticCurveTo(14, 26 + v * 3, 21, 36); g.stroke();
    for (let i = 0; i < 6; i++) { g.fillStyle = `rgba(120,135,160,${(0.08 + r() * 0.1).toFixed(3)})`; g.fillRect(r() * 46, r() * 46, 1.2, 1.2); }
  }
  // a street: grey-blue setts, small stones in staggered courses, darker than anything round them
  function patRoad(g, v) {
    const r = mulberry32(0x40ad + v * 211);
    g.fillStyle = '#566174'; g.fillRect(0, 0, 48, 48);
    const cols = ['#8793a8', '#909cb0', '#7f8ca2', '#98a3b6', '#8a96ab'];
    for (let row = 0; row < 6; row++) {
      const off = ((row + v) % 2) ? 6 : 0, y = row * 8;
      for (let c = -1; c < 5; c++) {
        const x = c * 12 + off;
        g.fillStyle = cols[Math.floor(r() * cols.length)]; g.fillRect(x + 1, y + 1, 10.4, 6.4);
        g.fillStyle = 'rgba(225,232,245,0.35)'; g.fillRect(x + 1, y + 1, 10.4, 1.2);
        g.fillStyle = 'rgba(30,38,55,0.22)'; g.fillRect(x + 1, y + 6.2, 10.4, 1.2);
      }
    }
  }
  ...
  function groundSprite(code, i, x) {
    ...
    switch (code) {
      ...
      case G_ROAD: return pat('road' + v, g => patRoad(g, v));
  ...
  function drawGroundRow(g, ty, x0, x1) {
    ...
    for (let tx = x0; tx <= x1; tx++) {
      ...
      if (code === G_ROAD) drawRoadEdges(g, tx, ty, x, y);
      if (CROWN_ISLE.has(tx, ty)) drawParapet(g, tx, ty, x, y);
  ...
  // a white stone kerb on the outside and a gold gutter just inside it
  const roadOn = (x, y) => paintsGround(x, y) && (GROUND[y * W + x] === G_ROAD || glyph(x, y) === '+' || glyph(x, y) === 'G');
  function drawRoadEdges(g, tx, ty, x, y) {
    for (const [dx, dy] of [[0, -1], [0, 1], [-1, 0], [1, 0]]) {
      if (roadOn(tx + dx, ty + dy)) continue;
      STATS.kerbs++;
      if (dy) { const ky = dy < 0 ? y : y + 43; g.fillStyle = '#f1ede4'; g.fillRect(x, ky, 48, 5); g.fillStyle = 'rgba(40,50,70,0.3)'; g.fillRect(x, dy < 0 ? ky + 5 : ky - 1, 48, 1); g.fillStyle = '#d9b44a'; g.fillRect(x, dy < 0 ? ky + 6 : ky - 3.5, 48, 2.5); }
      else { const kx = dx < 0 ? x : x + 43; g.fillStyle = '#f1ede4'; g.fillRect(kx, y, 5, 48); g.fillStyle = 'rgba(40,50,70,0.3)'; g.fillRect(dx < 0 ? kx + 5 : kx - 1, y, 1, 48); g.fillStyle = '#d9b44a'; g.fillRect(dx < 0 ? kx + 6 : kx - 3.5, y, 2.5, 48); }
    }
  }
  // the Crown: a low wall of white stone with a gold coping round the island's edge (the bridge end stays open)
  const CROWN_ISLE = { x0: 87, y0: 5, x1: 99, y1: 29, has(x, y) { return x >= this.x0 && x <= this.x1 && y >= this.y0 && y <= this.y1; } };
  function drawParapet(g, tx, ty, x, y) {
    for (const [dx, dy] of [[0, -1], [0, 1], [-1, 0], [1, 0]]) {
      if (!isSky(tx + dx, ty + dy)) continue;
      STATS.parapets++;
      if (dy < 0) { g.fillStyle = '#dcd4c2'; g.fillRect(x, y, 48, 9); g.fillStyle = '#f6f2ea'; g.fillRect(x, y, 48, 5); g.fillStyle = '#e8c25a'; g.fillRect(x, y, 48, 2); g.fillStyle = 'rgba(40,60,100,0.22)'; g.fillRect(x, y + 9, 48, 3); }
      else if (dy > 0) { g.fillStyle = '#f6f2ea'; g.fillRect(x, y + 34, 48, 5); g.fillStyle = '#dcd4c2'; g.fillRect(x, y + 39, 48, 9); g.fillStyle = '#e8c25a'; g.fillRect(x, y + 34, 48, 2); g.fillStyle = 'rgba(150,135,110,0.35)'; for (let k = 0; k < 4; k++) g.fillRect(x + k * 12 + 5, y + 40, 1, 8); }
      else { const kx = dx < 0 ? x : x + 40; g.fillStyle = '#dcd4c2'; g.fillRect(kx, y, 8, 48); g.fillStyle = '#f6f2ea'; g.fillRect(kx + (dx < 0 ? 0 : 3), y, 5, 48); g.fillStyle = '#e8c25a'; g.fillRect(kx + (dx < 0 ? 0 : 6), y, 2, 48); g.fillStyle = 'rgba(40,60,100,0.2)'; g.fillRect(dx < 0 ? kx + 8 : kx - 3, y, 3, 48); }
      // a post at each end of the stretch, gold-capped
      g.fillStyle = '#fbf8f2';
      if (dy) for (const px of [x + 1, x + 41]) { g.fillRect(px, dy < 0 ? y : y + 32, 6, dy < 0 ? 11 : 16); g.fillStyle = '#e8c25a'; g.fillRect(px, dy < 0 ? y : y + 32, 6, 2); g.fillStyle = '#fbf8f2'; }
      else for (const py of [y + 1, y + 41]) { g.fillRect(dx < 0 ? x : x + 39, py, 9, 6); g.fillStyle = '#e8c25a'; g.fillRect(dx < 0 ? x : x + 39, py, 9, 2); g.fillStyle = '#fbf8f2'; }
    }
  }
  // ---------- the ground, cached in chunks ----------
  // Nothing on the ground moves (the paving, lawn, marble, inlays, kerbs, bridge decks, doors and doorsteps), so it is
  // painted once into 8x8-tile canvases at the screen's pixel ratio and each frame only blits the few that are in view,
  // instead of five hundred tiles and their trimmings. A chunk is painted with a one-tile apron (a doorstep's glow reaches
  // into the next tile) and the least recently used are dropped past a small limit, so the memory stays bounded.
  const CH = 8, CHUNKS = new Map();
  let chunkMax = 20;          // at least every chunk one screen shows, and a few more: grown by drawGround when a big screen needs it
  let chunkUse = 0;
  function groundChunk(cx, cy) {
    ...
    if (!ch) {
      ...
      if (cg) {
        try {
          cg.scale(ss, ss); cg.translate(-cx * CH * TILE, -cy * CH * TILE);
          const x0 = Math.max(0, cx * CH - 1), x1 = Math.min(W - 1, cx * CH + CH), y0 = Math.max(0, cy * CH - 1), y1 = Math.min(H - 1, cy * CH + CH);
          for (let ty = y0; ty <= y1; ty++) drawGroundRow(cg, ty, x0, x1);
        } catch (e) { }
      }
      ch = { c, used: ++chunkUse }; CHUNKS.set(key, ch); STATS.chunksPainted++;
      // over the limit: drop the ones used longest ago (never this one, which was just marked as the newest)
  ...
  // the cached pieces of the sky decor: a cloud-top far below, the plaza's retaining face, the face under a bridge
  function paintPuff(g) {
    g.fillStyle = 'rgba(160,190,230,0.35)'; for (const [ox, oy, rr] of [[0, 6, 14], [16, 8, 12], [-14, 9, 10]]) { g.beginPath(); g.ellipse(ox, oy, rr * 1.3, rr * 0.7, 0, 0, 7); g.fill(); }
    g.fillStyle = 'rgba(255,255,255,0.78)'; for (const [ox, oy, rr] of [[0, 0, 14], [16, 3, 11], [-14, 4, 10], [6, -6, 9]]) { g.beginPath(); g.arc(ox, oy, rr, 0, 7); g.fill(); }
  }
  function paintFace(g, v) {
    g.fillStyle = '#e3dccd'; g.fillRect(0, 0, 48, 24);
    g.fillStyle = 'rgba(150,135,110,0.3)'; for (let r = 0; r < 2; r++) { g.fillRect(0, 11 + r * 12, 48, 1); for (let c = 0; c < 3; c++) g.fillRect(((r + v) % 2 ? 12 : 0) + c * 24, r * 12, 1, 11); }
    const sh = g.createLinearGradient(0, 24, 0, 44); sh.addColorStop(0, 'rgba(60,90,140,0.30)'); sh.addColorStop(1, 'rgba(60,90,140,0)'); g.fillStyle = sh; g.fillRect(0, 24, 48, 20);
  }
  function paintSpanFace(g, stone) {
    if (stone) { g.fillStyle = '#e3dccd'; g.fillRect(0, 0, 48, 16); g.fillStyle = '#8fd0ff'; g.beginPath(); g.ellipse(24, 16, 18, 10, 0, Math.PI, 0); g.fill(); g.fillStyle = '#e8c25a'; g.fillRect(0, 0, 48, 2); }
    const sh = g.createLinearGradient(0, 12, 0, 40); sh.addColorStop(0, 'rgba(60,90,140,0.22)'); sh.addColorStop(1, 'rgba(60,90,140,0)'); g.fillStyle = sh; g.fillRect(0, 12, 48, 28);
  }
  function drawSkyDecor(g, x0, x1, y0, y1) {
    ...
    for (let ty = y0 - 1; ty <= y1 + 1; ty++) for (let tx = x0 - 3; tx <= x1; tx++) {
      ...
      blit(g, sprite('puff', 72, 44, 36, 20, paintPuff), cx, cy);
    }
    ...
    for (let tx = Math.max(x0, PLAZA.x0); tx <= Math.min(x1, PLAZA.x1); tx++) {
      ...
      const v = tx % 2;
      blit(g, sprite('face' + v, 48, 44, 0, 0, cg => paintFace(cg, v)), tx * TILE, ty * TILE);
    }
    ...
    for (const b of PLAN.BRIDGES) for (const [bx0, by0, bx1, by1] of b.rects) {
      ...
      if (dir === 'ew') for (let tx = bx0; tx <= bx1; tx++) {
        ...
        const stone = b.kind === 'stone';
        blit(g, sprite('span' + (stone ? 's' : 'r'), 48, 40, 0, 0, cg => paintSpanFace(cg, stone)), tx * TILE, ty * TILE);
  ...
  // the wall's stonework never moves: each kind of wall tile is painted once (the brick courses alternate with the column,
  // and a north-south run has a cap where it ends) and blitted; only the banners wave, drawn over it
  function paintWallH(g, v) {
    g.fillStyle = 'rgba(40,60,100,0.15)'; g.fillRect(0, 48, 48, 9);
    // the face
    g.fillStyle = '#dad0bd'; g.fillRect(0, 14, 48, 34);
    g.fillStyle = 'rgba(145,130,105,0.28)';
    for (let r = 0; r < 3; r++) { const yy = 15 + r * 11; g.fillRect(0, yy + 10, 48, 1); const off = (r + v) % 2 ? 12 : 0; for (let c = 0; c < 3; c++) g.fillRect(off + c * 24, yy, 1, 10); }
    const sh = g.createLinearGradient(0, 30, 0, 48); sh.addColorStop(0, 'rgba(60,80,120,0)'); sh.addColorStop(1, 'rgba(60,80,120,0.16)'); g.fillStyle = sh; g.fillRect(0, 30, 48, 18);
    // the walk, the gold string course, the merlons (world-aligned, so the whole wall lines up)
    g.fillStyle = '#eae3d5'; g.fillRect(0, 3, 48, 9);
    g.fillStyle = '#e8c25a'; g.fillRect(0, 12, 48, 3); g.fillStyle = '#fff0b0'; g.fillRect(0, 12, 48, 1);
    for (let k = 0; k < 4; k += 2) {
      const mx = k * 12;
      g.fillStyle = '#e1d8c6'; g.fillRect(mx, -6, 12, 18);
      g.fillStyle = '#f0eadf'; g.fillRect(mx, -9, 12, 4);
      g.fillStyle = '#f5c542'; g.fillRect(mx, -10, 12, 2);
      g.fillStyle = 'rgba(60,80,120,0.12)'; g.fillRect(mx + 9, -6, 3, 18);
    }
  }
  ...
  // a fountain is two cached pieces (the basin with its still water, and the tiers or the statue) with the moving water drawn
  // between and over them each frame: the ripples, the water falling off every rim, the jets and the spray
  const FR = 70, FK = 0.84;
  function paintBasin(g) {
    g.fillStyle = 'rgba(40,60,100,0.18)'; g.beginPath(); g.ellipse(6, FR * FK + 6, FR, 12, 0, 0, 7); g.fill();
    // the basin: a rim face on the near side, the rim, the water
    octagon(g, 0, 10, FR, FK); g.fillStyle = '#d9d1c1'; g.fill();
    octagon(g, 0, 0, FR, FK); g.fillStyle = '#f4f0e7'; g.fill();
    g.strokeStyle = '#e8c25a'; g.lineWidth = 2.5; g.stroke();
    octagon(g, 0, 0, FR - 10, FK); const wat = g.createLinearGradient(0, -FR * FK, 0, FR * FK); wat.addColorStop(0, '#7fc6ef'); wat.addColorStop(1, '#2f8fd8'); g.fillStyle = wat; g.fill();
    g.fillStyle = 'rgba(255,255,255,0.25)'; g.beginPath(); g.ellipse(-26, -22, 16, 5, -0.3, 0, 7); g.fill();
  }
  function paintTiers(g) {
    // three tiers: the water falls up as much as it falls down
    g.fillStyle = '#ece6d9'; g.fillRect(-7, -24, 14, 30);
    bowl(g, 0, -24, 34, 11);
    g.fillStyle = '#ece6d9'; g.fillRect(-5, -50, 10, 26);
    bowl(g, 0, -50, 22, 7.5);
    g.fillStyle = '#ece6d9'; g.fillRect(-4, -70, 8, 20);
    bowl(g, 0, -70, 12, 4.5);
  ...
  function drawFountain(g, f) {
    ...
    blit(g, sprite('basin', 170, 150, 85, 62, paintBasin), cx, cy);
    ...
    g.restore();
    if (royal) {
      blit(g, sprite('tiers', 80, 112, 40, 102, paintTiers), cx, cy);
      curtain(g, cx, cy - 24, 34, 26, t); curtain(g, cx, cy - 50, 22, 24, t + 0.3); curtain(g, cx, cy - 70, 12, 18, t + 0.6);
  ...
  const drawHook = (g, items, c0) => {
    if (!inside()) return;
    const c = c0 || cam;
    STATS.frames++; STATS.chunks = 0; STATS.gates = 0; STATS.fountains = 0; STATS.awnings = 0; STATS.banners = 0; STATS.pennants = 0; STATS.spires = 0; STATS.towers = 0;
    ...
    // the ground: before every other ground-level thing (36 paints its sky and cloud over what is not ours; nothing of ours is under them)
    items.push({ y: -1e8 - 2, draw: () => drawGround(g, c) });
  ...
  if (window.KEYRING && KEYRING.register) KEYRING.register('lark_feather', "Lark's first feather. Walk up to a gold updraft stone in Aerie and use it (E, or tap it), and she flies you anywhere in the city.");
  ...
  if (window.WIKI && WIKI.add) {
    ...
    const lines = [
      ...
      { t: 'STREETS', c: '#8b949e' },
      'The Kingsway runs from the Great Gate to the Flight Gate. The Ring Road goes all the way round inside the wall.',
      `${PLAN.ROADS.filter(r => r.id !== 'kingsway' && r.id !== 'ring').map(r => r.name).join(', ')}.`,
      { t: 'GETTING ROUND', c: '#8b949e' },
      'Six updraft stones carry you between the Wind Landing, the Crown, the Underside and the Spire Run.',
      "Two more stones are edged in gold, at the Wind Landing and on the Royal Plaza. With Lark's first feather, walk up to either and use it (E, or tap it), and she flies you to any of six places in the city.",
    ...
    WIKI.add('items', { id: 'lark_feather', lines: [
      ...
      `Walk up to one of the two gold updraft stones (the Wind Landing and the Royal Plaza) and use it (E, or tap it): Lark flies you to ${DESTS.map(d => d.name).join(', ')}.`,
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    const setRecorder = () => { const log = []; const g = new Proxy({}, {
      get: (t, k) => k === 'measureText' ? (x => ({ width: String(x).length * 6 })) : (k === 'createLinearGradient' || k === 'createRadialGradient') ? (() => ({ addColorStop: () => { } })) : typeof k === 'string' ? ((...a) => { log.push([k, a]); }) : undefined,
    ...
    const meanFill = paintFn => { const { g, log } = setRecorder(); paintFn(g); const hs = log.filter(e => e[0] === '=fillStyle' && /^#[0-9a-f]{6}$/i.test(e[1][0])).map(e => e[1][0]); return hs.length ? hs.reduce((a, h) => a + lum(h), 0) / hs.length : -1; };

    // ---- K3b. the streets read as streets ----
    // Review round 1: 1,509 cells inside the walls were one cream brick, the same brick as the wall. A street is now its own
    // surface (grey-blue setts, a white kerb and a gold gutter on every edge) and the paving between streets pale flagstones.
    { const r = {}, KW = PLAN.ROADS.find(o => o.id === 'kingsway'), ROADC = G_ROAD;
      // every Kingsway paving cell is painted as street
      const kw = []; for (const [x0, y0, x1, y1] of KW.rects) for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) if (glyph(x, y) === '=') kw.push([x, y]);
      r.kingsway = kw.length >= 60 && kw.every(([x, y]) => GROUND[y * W + x] === ROADC && PLAN.roadAt(x, y) === KW); r.kwCells = kw.length;
      // and bordered by what is not street: beside it (x 46 and x 50) only a street that crosses it is street
      let bothOff = 0; const sideBad = [];
      for (const [x0, y0, x1, y1] of KW.rects) for (let y = y0; y <= y1; y++) {
        const sides = [x0 - 1, x1 + 1].map(x => GROUND[y * W + x] === ROADC ? PLAN.roadAt(x, y) : null);
        for (const sd of sides) if (sd && sd.id === 'kingsway') sideBad.push(y);
        if (sides.every(sd => !sd)) bothOff++;
      }
      r.bordered = sideBad.length === 0 && bothOff >= 14; r.bothOff = bothOff;
      // every street has street on it, and every street joins the Great Gate over street, gates and bridges
      const d = flood(inst().tiles, 48, 61, (t, x, y) => GROUND[y * W + x] === ROADC || glyph(x, y) === 'G' || glyph(x, y) === '+');
      const cut = PLAN.ROADS.filter(rd => { let any = false, all = true; for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) if (PLAN.roadAt(x, y) === rd && glyph(x, y) === '=') { any = true; if (d[y * W + x] < 0) all = false; } return !(any && all); }).map(rd => rd.name);
      r.joined = cut.length === 0 && PLAN.ROADS.length === 11; r.cut = cut;
      // no single surface is the ground inside the walls any more (it was one brick on half of it)
      const share = {}; let n = 0; for (let y = 12; y <= 60; y++) for (let x = 17; x <= 78; x++) { const c = GROUND[y * W + x]; if (!c || c === G_CWALL || c === G_HWALL) continue; share[c] = (share[c] || 0) + 1; n++; }
      const top = Math.max(...Object.values(share)) / n; r.mixed = top < 0.4; r.topShare = Math.round(top * 1000) / 10;
      // the setts are dark and the flagstones and the wall brick light: a street never looks like the wall beside it
      const road = meanFill(g => patRoad(g, 0)), pave = meanFill(g => patPave(g, 0)), brick = meanFill(g => patCwall(g, 0));
      r.contrast = road > 0 && road < 0.62 && pave > 0.85 && brick > 0.75 && pave - road > 0.25 && brick - road > 0.2; r.lum = [road, pave, brick].map(v => Math.round(v * 100) / 100);
      // drawn: a Kingsway row between two planters has a kerb on each outer side (none between its own cells), with the gold gutter
      { const k0 = STATS.kerbs, { g, log } = setRecorder(); drawGroundRow(g, 55, 47, 49); r.kerbs = STATS.kerbs - k0 === 2 && log.some(e => e[0] === '=fillStyle' && e[1][0] === '#d9b44a') && log.some(e => e[0] === '=fillStyle' && e[1][0] === '#f1ede4'); }
      // and a tap on it names it
      r.named = tapLabelFor({ kind: 'walk', t: PAVE, tx: 48, ty: 55 }) === 'Kingsway' && tapLabelFor({ kind: 'walk', t: PAVE, tx: 30, ty: 51 }) === 'Forge Street' && tapLabelFor({ kind: 'walk', t: PAVE, tx: 70, ty: 44 }) === 'Paving';
      check(K + 'K3b the streets read as streets: all 11 join the Great Gate; the Kingsway is setts from end to end with pavement or planters beside it (both sides off-street on 14+ rows); setts dark, flagstones and wall brick light; a kerb and a gold gutter on its edges; no surface covers 40% of the city',
        Object.values(r).every(v => v !== false), r); }

    // ---- K3c. the Crown is part of the kingdom ----
    // Review round 1: 240 of the Crown's 273 walkable cells were bare cloud. Now: the Rookery Yard in setts, perch-towers
    // with pennants, nest-houses on posts, a garden with beds, trees and lamps, a paved ring round the Songstone, a low
    // white-and-gold wall round the edge.
    { const r = {}, tl = inst().tiles, C = CROWN_ISLE; let walk = 0, cloud = 0, beds = 0, lamps = 0, trees = 0, benches = 0, open = 0;
      for (let y = C.y0; y <= C.y1; y++) for (let x = C.x0; x <= C.x1; x++) {
        const t = tl[y * W + x], k = kindAt(x, y);
        if (!SOLID.has(t)) walk++; if (t === SKYCITY.CLOUD) cloud++; if (t === BLOOM) beds++;
        if (k === 'lamp') lamps++; else if (k === 'tree') trees++; else if (k === 'bench') benches++;
        if (glyph(x, y) !== '~') for (const [dx, dy] of [[0, -1], [0, 1], [-1, 0], [1, 0]]) if (isSky(x + dx, y + dy)) open++;
      }
      r.noBareCloud = cloud === 0 && walk >= 190; r.counts = { walk, cloud, beds, lamps, trees, benches };
      r.garden = beds >= 12 && lamps >= 8 && trees >= 4 && benches === 2;
      const RK = PLAN.ROADS.find(o => o.id === 'rookery'); let yard = 0, yardOk = true;
      for (const [x0, y0, x1, y1] of RK.rects) for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) if (glyph(x, y) === '=') { yard++; if (GROUND[y * W + x] !== G_ROAD) yardOk = false; }
      r.rookeryYard = yardOk && yard === 48 && SP.perches.every(([x, y]) => GROUND[y * W + x] === G_ROAD && tl[y * W + x] === T.HAWK_PERCH);
      // the paved ring: every one of the 16 cells round the Songstone's 3x3 floor is street
      const R0 = SP.songRing; let ring = 0; for (let y = R0.y0 - 1; y <= R0.y1 + 1; y++) for (let x = R0.x0 - 1; x <= R0.x1 + 1; x++) { const edge = x === R0.x0 - 1 || x === R0.x1 + 1 || y === R0.y0 - 1 || y === R0.y1 + 1; if (edge && GROUND[y * W + x] === G_ROAD) ring++; }
      r.songRing = ring === 16;
      // the low wall: drawn on every side of the island that faces open sky (the bridge end stays open)
      { const p0 = STATS.parapets, { g } = recorder(); for (let y = C.y0; y <= C.y1; y++) drawGroundRow(g, y, C.x0, C.x1); r.parapet = STATS.parapets - p0 === open && open >= 40; r.parapetSides = [STATS.parapets - p0, open]; }
      // the perch-towers stand tall (over two tiles above their foot) and fly a pennant; the nest-houses have a roof over their posts
      { const art = AERIE.ART, a0 = art.perchTowers, n0 = art.nests, { g, log } = recorder(); const [px, py] = SP.perches[0];
        AERIE.drawPerch(g, px, py); const topY = Math.min(...log.filter(e => e[0] === 'fillRect' || e[0] === 'moveTo').map(e => e[1][1]));
        const { g: g2, log: log2 } = recorder(); const [nx, ny] = SP.nests[3]; AERIE.drawNest(g2, nx, ny); const roofY = Math.min(...log2.filter(e => e[0] === 'moveTo' || e[0] === 'lineTo').map(e => e[1][1]));
        r.towers = art.perchTowers - a0 === 1 && topY < (py + 1) * TILE - 2.5 * TILE && art.nests - n0 === 1 && roofY < ny * TILE - 20; r.art = { topY: topY - py * TILE, roofY: roofY - ny * TILE }; }
      // and it is all still walkable to: Pell, the three perches, the Songstone and both Crown updrafts from the bridge
      { const d = flood(tl, 87, 17, walkable); r.walked = reachAt(d, SP.pell[0], SP.pell[1]) >= 0 && SP.perches.every(([x, y]) => reachAt(d, x, y) >= 0) && reachAt(d, SP.songstone[0], SP.songstone[1]) >= 0 && reachAt(d, 96, 19) >= 0 && reachAt(d, 96, 27) >= 0; }
      check(K + 'K3c the Crown: no bare cloud left; the Rookery Yard in setts under the three perch-towers (over two and a half tiles tall, with a pennant); nest-houses with roofs; a garden of 12+ beds, 8+ lamps, 4+ trees and 2 benches; all 16 cells round the Songstone paved; a low wall on every side facing the sky; Pell, the perches, the Songstone and both updrafts in reach',
        Object.values(r).every(v => v !== false), r); }
    ...
    // ---- K13b. how the gold stones are used, said the same way everywhere ----
    // The stone is solid: you cannot stand on it. Walking into it does nothing; E or a tap uses it. Review round 1 found
    // "stand on" in four of the five places a child reads about it, so every line about the stones is gathered and read.
    { const r = {}, lines = [];
      const strings = o => { const out = [], seen = new Set(); const walk = (v, depth) => { if (typeof v === 'string') out.push(v); else if (v && typeof v === 'object' && depth < 4 && !seen.has(v)) { seen.add(v); (Array.isArray(v) ? v : Object.values(v)).forEach(u => walk(u, depth + 1)); } }; walk(o, 0); return out; };
      // the Queen when she pays, and Lark when she gives the feather
      quest.sky = { stage: 'done', wisps: 0, forged: 0 }; quest.kingdom = { stage: 6, wishes: 0, seen: {} }; larkFromStage(); featherOff(); enter(); closePanel(); drain();
      F.tp(48, 30); F.step([]); F.face(48, 29); F.press('KeyE'); F.sim(2, []); r.queen = Q().stage === 'done'; lines.push(...said().map(d => d.text)); drain();
      SCENE.active = true; SCENE.land = null; sceneFinish(); lines.push(...said().filter(d => d.who === 'Lark').map(d => d.text)); drain();
      // the quest log, the keyring and the book
      quest.kingdom = { stage: 'done', wishes: 0, seen: {} }; larkFromStage();
      lines.push(questText('lark'), KEYRING.KEYS.lark_feather ? KEYRING.KEYS.lark_feather.note : '');
      lines.push(...strings(WIKI.get('items', 'lark_feather')), ...strings(WIKI.get('places', 'aerie')));
      const about = lines.filter(t => /updraft stone|stones are edged in gold|gold stone/i.test(t) && /(gold|feather|Lark)/.test(t));
      r.found = about.length >= 5;
      r.noStandOn = about.every(t => !/\bstand (on|at)\b/i.test(t));
      r.useIt = about.every(t => /\buse it\b/i.test(t));
      r.keysTold = [questText('lark'), KEYRING.KEYS.lark_feather.note].every(t => /\(E, or tap it\)/.test(t));
      restoreKit(); closePanel(); drain();
      check(K + 'K13b the gold stones are explained the same way everywhere a child reads about them (the Queen, Lark, the quest log, the keyring, the book): walk up to one and use it (E, or tap it); never "stand on"',
        Object.values(r).every(Boolean), Object.assign(r, { about })); }
    ...
    // ---- K22. the ground is painted once and blitted: standing still paints no chunk again, and the cache stays bounded ----
    { enter(); const iw = window.innerWidth, ih = window.innerHeight; window.innerWidth = 1280; window.innerHeight = 800;
      F.tp(48, 46); F.step([]); render(); const p0 = STATS.chunksPainted; render(); render(); const again = STATS.chunksPainted - p0, blits = STATS.chunks;
      for (const [x, y] of [[20, 14], [78, 60], [90, 76], [10, 70], [48, 2], [95, 10]]) { F.tp(x, y); F.step([]); render(); }
      const bounded = CHUNKS.size <= chunkMax && chunkMax <= 40;
      window.innerWidth = iw; window.innerHeight = ih; render();
      check(K + 'K22 the ground is cached: standing still repaints no chunk (it only blits the 12 or so in view), and after walking the whole city the cache holds no more than it may',
        again === 0 && blits > 0 && blits <= 20 && bounded, { again, blits, cached: CHUNKS.size, max: chunkMax }); }
