...
// ============================================================================
{
  ...
  const BLUE = '#6fb1ff';        // a friend's name: the kit's friend blue (src/59-hudkit.js T.friend)
  ...
  const nameColour = l => l.role === 'admin' ? GOLD : l.n === me() ? HK.T.ink : BLUE;
...
#chatbox .fl-chat-send:active{transform:translateY(1.5px);background:linear-gradient(180deg,#16181c,#2d3138);box-shadow:inset 0 0 0 1.5px rgba(217,178,92,.6)}`;
  ...
  function drawBubble(g, b, x, y) {
    ...
    // an admin's bubble wears a gold edge over the plate's outline (docs/ONLINE.md, "Roles"); a player's has the plain vellum
    if (b.role === 'admin') { HK.rr(g, bx - 0.5, by - 0.5, bw + 1, bh + 1, 5); g.strokeStyle = GOLD; g.lineWidth = 1.5; g.stroke(); }
  ...
  function drawStrip(g) {
    ...
    lines.forEach((l, i) => {
      ...
      const sys = l.n === null, nf = HK.FS(700, 13), tf = HK.FS(sys ? 700 : 600, 13), name = sys ? '' : l.n + ':';
      const nw = sys ? -6 : HK.tw(g, name, nf), maxT = lane.w - 20 - nw - 6;
      ...
      if (!sys) HK.text(g, name, lane.x + 8, y + 14, { font: nf, color: nameColour(l) });
      HK.text(g, t, lane.x + 8 + nw + 6, y + 14, { font: tf, color: sys ? (l.color || HK.T.ink) : HK.T.ink });
      g.restore();
    });
  ...
  HOOKS.panel.chatlog = (g, narrow) => {
    ...
    for (let i = log.length - 1; i >= 0; i--) {
      // a system line (an admin's word, a party hat found) has no name: its words run the whole width, in its own colour
      const l = log[i], sys = l.n === null, own = !sys && HK.tw(g, l.n, nf) > nameCol, lines = bubbleLines(g, l.text, own || sys ? iw - 24 : textW, tf, 4);
    ...
    let y = py + top + pad;
    rows.forEach((r, k) => {
      const sys = r.l.n === null, tx = r.own || sys ? x0 + 12 : x0 + 12 + nameCol + 10;
      // a name wears its role's colour (an admin's is gold, yours is ink, a friend's is friend blue)
      if (!sys) HK.text(g, r.l.n, x0 + 12, y + lh - 3, { font: nf, color: nameColour(r.l), shadow: 'rgba(0,0,0,0.9)', box: r.own ? { x: x0 + 12, y, w: iw - 24, h: r.h } : { x: x0 + 12, y, w: nameCol, h: r.h }, fitId: 'chatlog:name' });
      r.lines.forEach((l, i) => HK.text(g, l, tx, y + (r.own ? lh : 0) + (i + 1) * lh - 3, { font: sys ? HK.FS(700, 13) : tf, color: sys ? (r.l.color || T.ink) : T.ink, box: { x: tx, y, w: r.own || sys ? iw - 24 : textW, h: r.h }, fitId: 'chatlog:text' }));
  ...
  HOOKS.selfTest.push((check, F, h) => {
    ...
    { close(); closePanel(); log.length = 0; for (const n in bubbles) delete bubbles[n];
      ...
      // the bubbles are the kit's vellum (outline + gold hairline): an admin's has a gold edge on top, a player's has none, and neither is blue
      const blueIn = e => e.some(c => /#7ec8ff|#6fb1ff/i.test(String(c)));
      const goldBubble = mudEdge.length > 0 && mudEdge.includes(GOLD) && avaEdge.length > 0 && !avaEdge.includes(GOLD) && !blueIn(mudEdge) && !blueIn(avaEdge);
      check(P + "roles: an admin's chat name is gold on the strip and in the log and their bubble has a gold edge; a line whose role is not 'admin' reads as a player's", roles && goldStrip && goldLog && goldBubble, { roles, goldStrip, goldLog, goldBubble, strip: strip.map(t => t.text + '=' + t.fill) }); rec.length = 0; }
    ...
    { log.length = 0; for (const n in bubbles) delete bubbles[n]; closePanel();
      ...
      check(P + 'CHAT.system writes a line with no name in its colour (gold by default) on the strip and in the log, with no bubble', ok && ok2 && empty === false && log.length === 2 && a.n === null && a.text === 'Sam found a purple party hat!' && a.color === GOLD && a.t > 0 && b.color === '#8b949e' && Object.keys(bubbles).length === 0 && onStrip, { ok, ok2, empty, a, b, onStrip, drawn: drawn.map(t => t.text + '=' + t.fill) }); }
