...
const buttons = []; // on-screen hit rects: {x,y,w,h,label,action} plus, optionally: r (+cx, cy) = a circle hit; up = fire on pointer-up inside; hold = a hold target (HK.held); inert = swallows the press and does nothing; name / keys = its tooltip
let minimapRect = null;
// A hit test that knows circles: a round control is hit inside its circle, not its bounding box.
function hitButton(b, x, y) {
  if (b.r) { const cx = b.cx != null ? b.cx : b.x + b.w / 2, cy = b.cy != null ? b.cy : b.y + b.h / 2; return Math.hypot(x - cx, y - cy) <= b.r; }
  return x >= b.x && x <= b.x + b.w && y >= b.y && y <= b.y + b.h;
}
// Order: on-screen buttons (last registered = drawn on top = wins) → pause menu swallows the rest → minimap → the dialogue box
// (dialogRect; never the joystick zone) → an open panel (inside: absorbed, outside: closes) → joystick (touch, left half) →
...
// Buttons marked `up` fire on pointer-up inside (slide off to cancel; held 400 ms they name themselves instead, src/59-hudkit.js);
// the rest (SWING, USE, BLOCK, the context seat, the pouches) fire on pointer-down. Either way the press shows in the next frame.
function pointerDown(x, y, id) {
  ...
  if (paused) return;
  if (minimapRect && (minimapRect.r ? Math.hypot(x - minimapRect.cx, y - minimapRect.cy) <= minimapRect.r : inRect(x, y, minimapRect))) { panel === 'map' ? closePanel() : openPanel('map'); return; }
...
function pointerMove(x, y, id) {
  if (typeof HK !== 'undefined') HK.pressMove(x, y, id);
...
function pointerUp(id, x, y) {
  if (typeof HK !== 'undefined' && HK.pressEnd(id, x, y)) return;
...
canvas.addEventListener('touchend', e => { e.preventDefault(); for (const t of e.changedTouches) pointerUp(t.identifier, t.clientX, t.clientY); }, { passive: false });
canvas.addEventListener('touchcancel', e => { for (const t of e.changedTouches) pointerUp(t.identifier, -1e4, -1e4); });
...
canvas.addEventListener('mouseleave', () => pointerMove(-1e4, -1e4, 'mouse'));
window.addEventListener('mouseup', e => pointerUp('mouse', e && e.clientX, e && e.clientY));
// The safe areas: a hidden fixed probe padded by env(safe-area-inset-*) (src/page.html), read on every resize and
// rotation. The HUD keeps max(these, each family's floor) clear of anything tappable (src/59-hudkit.js).
function readSafeArea() {
  try {
    const el = document.getElementById('safe-probe'); if (!el || typeof getComputedStyle !== 'function') return;
    const cs = getComputedStyle(el), n = v => Math.max(0, parseFloat(v) || 0);
    if (typeof HK !== 'undefined') Object.assign(HK.SAFE_ENV, { t: n(cs.paddingTop), r: n(cs.paddingRight), b: n(cs.paddingBottom), l: n(cs.paddingLeft) });
  } catch (e) { }
}
window.addEventListener('resize', () => readSafeArea());
window.addEventListener('orientationchange', () => setTimeout(readSafeArea, 50));
